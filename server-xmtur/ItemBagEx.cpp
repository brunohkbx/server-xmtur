...
BOOL CItemBagEx::DropRedRibbonBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.RedRibbonBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.RedRibbonBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT CHRISTMAS RIBBONBOX] RIBBONBOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.RedRibbonBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropGreenRibbonBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.GreenRibbonBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.GreenRibbonBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT CHRISTMAS RIBBONBOX] GREENRIBBONBOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.GreenRibbonBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropBlueRibbonBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.BlueRibbonBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.BlueRibbonBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT CHRISTMAS RIBBONBOX] BLUERIBBONBOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.BlueRibbonBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropPinkChocolateBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.PinkChocolateBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.PinkChocolateBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT VALENTINE'S DAY CHOCOLATEBOX] PINKCHOCOLATE BOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.PinkChocolateBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropRedChocolateBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.RedChocolateBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.RedChocolateBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT VALENTINE'S DAY CHOCOLATEBOX] REDCHOCOLATEBOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.RedChocolateBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropBlueChocolateBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.BlueChocolateBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.BlueChocolateBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT VALENTINE'S DAY CHOCOLATEBOX] BLUECHOCOLATEBOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.BlueChocolateBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropLightPurpleCandyBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.LightPurpleCandyBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.LightPurpleCandyBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT WHITE DAY CANDY BOX] LIGHT PURPLE BOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.LightPurpleCandyBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropVermilionCandyBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.VermilionCandyBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.VermilionCandyBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT WHITE DAY CANDY BOX] VERMILION BOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.VermilionCandyBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropDeepBlueCandyBoxEventItem(int aIndex)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( DropItemRate < Configs.DeepBlueCandyBoxDropZenRate )
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.DeepBlueCandyBoxDropZen, 
				lpObj->X, lpObj->Y);

			LogAddTD("[EVENT WHITE DAY CANDY BOX] DEEP BLUE BOX Event ItemDrop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name,
				Configs.DeepBlueCandyBoxDropZen);
		}
		else
		{
			DropItemNum = rand() % this->GetBagCount();
...
BOOL CItemBagEx::DropHallowinEventItem(LPOBJ lpObj)
{
	...
	if ( iItemBagCount > 0 )
	{
		iTotalDropRate = Configs.HallowinDayEventJOLBlessDropRate +
			             Configs.HallowinDayEventJOLAngerDropRaTe +
						 Configs.HallowinDayEventJOLScreamDropRate +
						 Configs.HallowinDayEventJOLFoodDropRate +
						 Configs.HallowinDayEventJOLDrinkDropRate +
						 Configs.HallowinDayEventJOLPolymorphRingDropRate;

		iDropItemRate = rand() % iTotalDropRate;

		if ( iDropItemRate >= (iTotalDropRate - Configs.HallowinDayEventJOLBlessDropRate)  )
			iDropItemNum = 0;
		else if ( iDropItemRate >= (iTotalDropRate - Configs.HallowinDayEventJOLBlessDropRate - Configs.HallowinDayEventJOLAngerDropRaTe) )
			iDropItemNum = 1;
		else if ( iDropItemRate >= (iTotalDropRate - Configs.HallowinDayEventJOLBlessDropRate - Configs.HallowinDayEventJOLAngerDropRaTe - Configs.HallowinDayEventJOLScreamDropRate) )
			iDropItemNum = 2;
		else if ( iDropItemRate >= (iTotalDropRate - Configs.HallowinDayEventJOLBlessDropRate - Configs.HallowinDayEventJOLAngerDropRaTe - Configs.HallowinDayEventJOLScreamDropRate - Configs.HallowinDayEventJOLFoodDropRate) )
			iDropItemNum = 3;
		else if ( iDropItemRate >= (iTotalDropRate - Configs.HallowinDayEventJOLBlessDropRate - Configs.HallowinDayEventJOLAngerDropRaTe - Configs.HallowinDayEventJOLScreamDropRate - Configs.HallowinDayEventJOLFoodDropRate - Configs.HallowinDayEventJOLDrinkDropRate) )
			iDropItemNum = 4;
		else if ( iDropItemRate >= (iTotalDropRate - Configs.HallowinDayEventJOLBlessDropRate - Configs.HallowinDayEventJOLAngerDropRaTe - Configs.HallowinDayEventJOLScreamDropRate - Configs.HallowinDayEventJOLFoodDropRate - Configs.HallowinDayEventJOLDrinkDropRate - Configs.HallowinDayEventJOLPolymorphRingDropRate) )
			iDropItemNum = 5;
...
BOOL CItemBagEx::DropCrywolfDarkElfItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( iDropItemRate < Configs.CrywolfMonsterDarkElfDropZenRate ) 
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.CrywolfMonsterDarkElfDropZen, cX, cY);

			LogAddTD("[ Crywolf ][Reward] Dark Elf Item Drop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name, Configs.CrywolfMonsterDarkElfDropZen);
		}
		else
		{
...
BOOL CItemBagEx::DropCrywolfBossMonsterItem(int aIndex, BYTE btMapNumber, BYTE cX, BYTE cY)
{
	...
	if ( this->GetBagCount() > 0 )
	{
		...
		if ( iDropItemRate < Configs.CrywolfBossMonsterDropZenRate ) 
		{
			MapC[lpObj->MapNumber].MoneyItemDrop(Configs.CrywolfBossMonsterDropZen, cX, cY);

			LogAddTD("[ Crywolf ][Reward] Boss Monster Item Drop [%s][%s] [%d Zen]",
				lpObj->AccountID, lpObj->Name, Configs.CrywolfBossMonsterDropZen);
		}
		else
		{
			iDropItemNum = rand()% this->GetBagCount();
