		...
		if(itype != -1)
		{
			...
			/*if(ItemDbByte >= 10)
			{
				...
				else
				{
					...
					{
						item.m_Option3 = Configs.JewelOfLifeMaxOption;
					}
				}
			}*/
			...
			else
			{
				if(_type >= ITEMGET(14,0) && _type <= ITEMGET(14,8)	|| _type >= ITEMGET(14,38) && _type <= ITEMGET(14,40))
				{
					...
					if(item.m_Durability > 3.0f && Configs.RemovePotionLimit == 0)
					{
			...
			if(_type == ITEMGET(14,70) || _type == ITEMGET(14,71))
			{
				...
				if(item.m_Durability > 50.0f && Configs.RemovePotionLimit == 0)
				{
...
void gObjSetBP(int aIndex)
{
	...
	switch ( gObj[aIndex].Class )
	{
		...
		case CLASS_RAGEFIGHTER:
			gObj[aIndex].MaxBP = (Strength * 0.15) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 0.1);
			break;
	}
}
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	if ( lpMsg->Class == 0x00 || lpMsg->Class == 0x10 || lpMsg->Class == 0x20 || lpMsg->Class == 0x30 
	|| lpMsg->Class == 0x01 || lpMsg->Class == 0x11 || lpMsg->Class == 0x21 || lpMsg->Class == 0x40 
	|| lpMsg->Class == 0x03 || lpMsg->Class == 0x13 || lpMsg->Class == 0x23 || lpMsg->Class == 0x32 
	|| lpMsg->Class == 0x42 || lpMsg->Class == 0x50 || lpMsg->Class == 0x51 || lpMsg->Class == 0x53
	|| lpMsg->Class == 0x60 || lpMsg->Class == 0x62)
	{
	...
	if ( lpObj->m_bMapSvrMoveReq == true )
	{
		...
		if ( sSVR_CODE == Configs.GameServerCode )
		{
	...
	if(Configs.BlockKanturuMapEnter == 1)
	{
	...
	if(Configs.BlockCastleSiegeMapEnter == 1)
	{
		...
		//Remove Non Duelers from Arena
	if(gObj[aIndex].MapNumber == 64){ 
		gObjMoveGate(aIndex,17);
		LogAdd("[DuelManager][%s][%s] Player Moved from Duel Arena",gObj->AccountID,gObj->Name);
	}

	//Remove Users from Doppelganger Event
	if(gObj[aIndex].MapNumber >= 65 && gObj[aIndex].MapNumber <= 68){
		gObjMoveGate(aIndex,267);
		LogAdd("[Doppelganger][%s][%s] Player Moved from Event Map",gObj->AccountID,gObj->Name);
	}
	
	//Remove Users from Imperial Guardian Event
	if(gObj[aIndex].MapNumber >= 69 && gObj[aIndex].MapNumber <= 72){
		gObjMoveGate(aIndex,22);
		LogAdd("[ImperialGuardian][%s][%s] Player Moved from Event Map",gObj->AccountID,gObj->Name);
	}
	...
	//Reset Send
	PMSG_SEND_RESETS pMsg;

	pMsg.H = 0xC1;
	pMsg.Size = sizeof(pMsg);
	pMsg.Head = 0xF3;
	pMsg.Subhead = 0x88; //Changed from 0x50
	pMsg.Resets = lpObj->Resets;

	DataSend(aIndex,(UCHAR*)&pMsg,pMsg.Size);

	LogAddTD("[Send Resets][%s][%s] Resets: %d",lpObj->AccountID,lpObj->Name,lpObj->Resets);
	...
	if ( ::Configs.ApplyHeroSystem != FALSE )
	{
	...
	if ( lpObj->Class == CLASS_WIZARD || lpObj->Class == CLASS_ELF || lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_DARKLORD 
		|| lpObj->Class == CLASS_SUMMONER || lpObj->Class == CLASS_MAGUMSA || lpObj->Class == CLASS_RAGEFIGHTER )
	{
		...
		else if ( lpObj->Class == CLASS_KNIGHT || lpObj->Class == CLASS_RAGEFIGHTER)
		{
	...
	if ( lpObj->Level <=5 && Configs.FixPStoreLevel5Remove == 0)
	{
		for ( int i=MAIN_INVENTORY_SIZE;i<INVENTORY_SIZE;i++)
		{
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 1, "Please Check Items in your Inventory!");
		DataSend(lpObj->m_Index, (LPBYTE)&pNotice, pNotice.h.size);
	}

	if ( lpObj->Level <= Configs.CharacterRecuperationMaxLevel )
	{
	...
	//Clear Quest T Info
	BYTE Packet_Quest[6]={0xC1,0x05,0xF6,0x0F,0x255,0x00};
	DataSend(aIndex,&Packet_Quest[0],Packet_Quest[1]);

	QuestManager.GetQuestList(aIndex);

	GensSystem.GetGensInformation(aIndex);

	ExtremePoints.GetExtremePoints(aIndex);

	MarrySystem.GetMarryInfo(aIndex);

	ReqWarehouseInUse(aIndex);

	//ConnectMessage
	GCServerMsgStringSend(Configs.ConnectMessage,aIndex,0);

	//Crea Mensaje para Enviar
	char Temp[255];
	sprintf_s(Temp,"%s esta Conectado",lpObj->Name);

	int GMNumber = GMManager.GMListNumber(lpObj->Name);

	if(lpObj->Authority == 32 && GMNumber != -1){
		if(GMManager.GMFeatures[GMNumber].ConnectMessage != 1){
			return TRUE;
		}
	}

	GCSendChatMessage(Temp);
	return TRUE;
}
...
void GCSendChatMessage(char* Message){
	...
	pMsg.h.size = (strlen(Message)+ 0x13);
...
BOOL gObjSetMonster(int aIndex, int MonsterClass)
{
	...
	{
		if(Configs.NewYearLuckyBagMonsterEventOn != FALSE)
		{
			lpObj->Live = TRUE;
			lpObj->m_State = 1;
		}
	...
	if(MonsterClass == 379 || MonsterClass == 385 || //Season 2.5 add-on 
	MonsterClass == 380 || MonsterClass == 381 || MonsterClass == 382 || MonsterClass == 383 || 
	MonsterClass == 384 || MonsterClass == 415 || MonsterClass == 416 ||  MonsterClass == 417 || MonsterClass == 450 
	|| MonsterClass == 478  || MonsterClass == 479 || MonsterClass == 547 || MonsterClass == 543 
	|| MonsterClass == 544 || MonsterClass == 546 //Gatekeeper Titus, Julia, Vernet & Duprian NPC (S5), Raul Jeweler
	|| MonsterClass == 540 || MonsterClass == 541 || MonsterClass == 542 || MonsterClass == 522
	|| MonsterClass == 492 || MonsterClass == 545){
		lpObj->Type = OBJ_NPC;
	}
...
bool gObjLevelUp(LPOBJ lpObj, __int64 & addexp, int iMonsterType, int iEventType) //0051F140
{
	...
	if(lpObj->Level <= 150) //season4.5 add-on good
	{
		if(gObjCheckIsInPCBang(lpObj) != FALSE)
		{
			...
			RewardEXP = (RewardEXP < Configs.LevelExperience[lpObj->Level])?(RewardEXP):(Configs.LevelExperience[lpObj->Level]);
	...
	if ( lpObj->Class == 4 )
	{
		lpObj->LevelUpPoint += Configs.DLPoints;
	}
	else if ( lpObj->Class == 3 )
	{
		lpObj->LevelUpPoint += Configs.MGPoints;
	}
	else
	{
...
{
	if (lpObj->Level < Configs.GuildCreateLevel)
	{
		return 0;
	}
	return 1;
...
{
	lpObj->NextExp = Configs.LevelExperience[lpObj->Level];
}
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj, int MSBDamage)
{
	...
	if(Configs.PKLevelIncreaseOff == 1)
	{
		return;
	}
	...
	if(lpTargetObj->PartyNumber >= 0 && Configs.FixRemovePKPartySystem == 0) //Season 2.5 add-on
	{
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	__int64 minexp = Configs.LevelExperience[-1+lpObj->Level];
	__int64 maxexp = Configs.LevelExperience[lpObj->Level];
 ...
 //Check Position in gs 77 after siege patch
	if(g_Crywolf.GetCrywolfState() == 4)
	{
	...
	if(lpObj->WarehouseMoney > 0)
	{
		...
		if(iMoneyLevel > 0)
		{
			submoney = (DWORD)(lpObj->WarehouseMoney * iMoneyLevel) / 100; //WZ Bug on Season3 (should be lpObj->WarehouseMoney like it was)
		}
...
void gObjPkDownTimeCheck(LPOBJ lpObj, int TargetLevel)
{
	...
	if(lpObj->m_PK_Level == 100)
	{
		return;
	}

	DWORD dwtime = ((GetTickCount() - lpObj->m_dwPKTimer) / 1000) * Configs.PkTime;
...
BOOL gObjWingSprite(LPOBJ lpObj)
{
	...
	if ( (Wing->m_Type >= ITEMGET(12,0) && Wing->m_Type <= ITEMGET(12,6)) || Wing->m_Type <= ITEMGET(13,30) || 
		 ...
		 (Wing->m_Type >= ITEMGET(12,180) && Wing->m_Type <= ITEMGET(12,183)) ||//Season 2.5 Third Wing add-on
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	if(sprite->m_Type == ITEMGET(13,64) || sprite->m_Type == ITEMGET(13,65)
	|| sprite->m_Type == ITEMGET(13,80) || sprite->m_Type == ITEMGET(13,106) || sprite->m_Type == ITEMGET(13,123)){
		fN += 10.0f;
	}

	if(sprite->m_Type == ITEMGET(13,80) || sprite->m_Type == ITEMGET(13,106) || sprite->m_Type == ITEMGET(13,123)){
		fdamage = (damage*3)/10.0f;
		fdamage /= fN;
		sprite->m_Durability -= fdamage;
		send_dur = 1;
	...
	else if(sprite->m_Type == ITEMGET(13,37))
	{
		...
		int iMaxDurSmall = Configs.FenrirDefaultMaxDurSmall;
		int iMultipleDmg = 2;
		...
		{
			iMaxDurSmall = Configs.FenrirElfMaxDurSmall;
			iMultipleDmg = 2;
		}
		else if(lpObj->Class == CLASS_MAGUMSA)
		{
...
int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp) //005289E0
{
	...
	{
		mymaxexp = Configs.LevelExperience[lpObj->Level];
	}
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	if(viewplayer > 1)
	{
		if(bApplaySetParty != 0)
		{
			if(viewplayer == 3)
			{
				viewpercent = Configs.PartyExp_03;
			}
			else if(viewplayer == 4)
			{
				viewpercent = Configs.PartyExp_04;
			}
			else if(viewplayer >= 5)
			{
				viewpercent = Configs.PartyExp_05;
			}
			else
			{
				viewpercent = Configs.PartyExp_Default;
			}
		}
		else
		{
			if(viewplayer == 2)
			{
				viewpercent = Configs.SetPartyExp_02;
			}
			else if(viewplayer == 3)
			{
				viewpercent = Configs.SetPartyExp_03;
			}
			else if(viewplayer == 4)
			{
				viewpercent = Configs.SetPartyExp_04;
			}
			else if(viewplayer >= 5)
			{
				viewpercent = Configs.SetPartyExp_05;
			}
			else
			{
				viewpercent = Configs.SetPartyExp_Default;
			}
		}
		...
		if(number >= 0)
		{
			...
			if(lpTargetObj->MapNumber == lpPartyObj->MapNumber && dis[n] < 10)
			{
				...
				else
				{
					...
					{
						mymaxexp = Configs.LevelExperience[lpPartyObj->Level];
					}
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, WORD Skill, int iShieldDamage)
{
	...
	if(lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
		...
		//Doppelganger Check
		if(lpTargetObj->Type == OBJ_MONSTER && DOPPELGANGER_MAP_RANGE(lpObj->MapNumber)){

			for(int X=0; X < Doppelganger.MonstersCount;X++){

				if(lpTargetObj->m_Index == Doppelganger.CurrentMonster[X]){
					Doppelganger.CurrentMonster[X] = -1;
					Doppelganger.MonstersCount--;
					gObjDel(lpTargetObj->m_Index);
				}
			}
		}
				
		if(lpTargetObj->Type == OBJ_MONSTER && IMPERIAL_MAP_RANGE(lpObj->MapNumber)){

			//Monsters Count - Imperial Guardian
			for(int X=0; X < ImperialGuardian.TotalMonsters;X++){

				if(lpTargetObj->m_Index == ImperialGuardian.SetedMonsters[X] && ImperialGuardian.SetedMonsters[X] > 0){
					ImperialGuardian.CurrentMonsters--;
					ImperialGuardian.SetedMonsters[X] = -1;
				}
			}

			//Imperial Guardian - Doors Die
...
{
	return Configs.ItemNumberCount;
}
...
BYTE gObjInventoryInsertItemPos(int aIndex, CItem item, int pos, BOOL RequestCheck)
{
	...
	if(pos < 12)
	{
		...
		if(lpItem->m_Option1)
		{
			int result = gObjWeaponMagicAdd(lpObj,(WORD)lpItem->m_Special[0],lpItem->m_Level);
	...
	__try
	{
		...
		case 14: //season4 add-on
			...
			if(sFlag == 0)
			{
				...
				else
				{
					...
					if(sitem->m_Level < 4 && sitem->m_Option3*4 < Configs.JewelOfLifeMaxOption)
					{
						if( sitem->m_Type == ITEMGET(12,15) || 
							sitem->m_Type == ITEMGET(14,13) || 
							sitem->m_Type == ITEMGET(14,14) || 
		...
		case 4:
			...
			if(target < INVETORY_WEAR_SIZE)
			{
				...
				if(lpItem->m_Option1)
				{
					int s_pos = gObjWeaponMagicAdd(&gObj[aIndex],(WORD)lpItem->m_Special[0],lpItem->m_Level);
					if(s_pos >= 0)
					{
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pWarehouse[target].m_Type == ITEMGET(13,76)) //Season 4.0 XMas add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
							if(lpObj->pWarehouse[target].m_Type == ITEMGET(13,122)) //Season 4.0 XMas add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
				...
				if(sFlag == 0)
				{
					if(source < INVETORY_WEAR_SIZE)
					{
						if(source == 10 || source == 11)
						{
							...
							if(lpObj->pChaosBox[target].m_Type == ITEMGET(13,76)) //Season 2.5 add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
							if(lpObj->pChaosBox[target].m_Type == ITEMGET(13,122)) //Season 2.5 add-on
							{
								gObjUseSkill.SkillChangeUse(aIndex);
							}
						}
		...
		if(sFlag == 0 && source < INVETORY_WEAR_SIZE)
		{
			if(lpObj->pInventory[source].IsItem() == 1)
			{
				...
				if(lpObj->pInventory[source].m_Type == ITEMGET(13,76)) //Season 2.5 add-on
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
				if(lpObj->pInventory[source].m_Type == ITEMGET(13,122)) //Season 2.5 add-on
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
			...
			else
			{
				if(source == 10 || source == 11)
				{
					...
					if(lpObj->pInventory[target].m_Type == ITEMGET(13,76)) //Season 2.5 add-on
					{
						gObjUseSkill.SkillChangeUse(aIndex);
						LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
					}
					if(lpObj->pInventory[target].m_Type == ITEMGET(13,122)) //Season 2.5 add-on
					{
						gObjUseSkill.SkillChangeUse(aIndex);
						LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
					}
		...
		if(tFlag == 0 && target < INVETORY_WEAR_SIZE)
		{
			...
			if(lpObj->pInventory[target].m_Type == ITEMGET(13,122)) //Season 4 add-on
			{
				gObjUseSkill.SkillChangeUse(aIndex);
				LogAdd(lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[target].m_Level);
			}
		...
		if(source < INVETORY_WEAR_SIZE)
		{
			if(source == 10 || source == 11)
			{
				if( lpObj->Trade[blank].m_Type == ITEMGET(13,10) ||
					...
					lpObj->Trade[blank].m_Type == ITEMGET(13,76) ||
					lpObj->Trade[blank].m_Type == ITEMGET(13,122))
				{
					gObjUseSkill.SkillChangeUse(lpObj->m_Index);
				}
			}
...
void gObjMakePreviewCharSet(int aIndex)
{
    ...
    if(lpObj->pInventory[8].m_Type != ITEMGET(13,4)){
        ...
        } else if(lpObj->pInventory[8].m_Type != ITEMGET(13,67) && lpObj->pInventory[8].m_Type != ITEMGET(13,123)){ //Rudolph & Skeleton
    ...
    lpObj->pInventory[7].m_Type == ITEMGET(13,30) || lpObj->pInventory[7].m_Type == ITEMGET(12,41) ||
    ...
    if(lpObj->pInventory[2].IsExtItem() != FALSE) lpObj->CharSet[10] = (char)0x80;
    if(lpObj->pInventory[3].IsExtItem() != FALSE) lpObj->CharSet[10] |= 0x40;
    if(lpObj->pInventory[4].IsExtItem() != FALSE) lpObj->CharSet[10] |= 0x20;
    if(lpObj->pInventory[5].IsExtItem() != FALSE) lpObj->CharSet[10] |= 0x10;
    if(lpObj->pInventory[6].IsExtItem() != FALSE) lpObj->CharSet[10] |= 0x8;
    if(lpObj->pInventory[0].IsExtItem() != FALSE) lpObj->CharSet[10] |= 0x04;
    if(lpObj->pInventory[1].IsExtItem() != FALSE) lpObj->CharSet[10] |= 0x02;
    
    //Set Set Items
    lpObj->CharSet[11] = 0;
    
    if(lpObj->pInventory[2].IsSetItem() != FALSE) lpObj->CharSet[11] = (char)0x80;
    if(lpObj->pInventory[3].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x40;
    if(lpObj->pInventory[4].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x20;
    if(lpObj->pInventory[5].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x10;
    if(lpObj->pInventory[6].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x08;
    if(lpObj->pInventory[0].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x04;
    if(lpObj->pInventory[1].IsSetItem() != FALSE) lpObj->CharSet[11] |= 0x02;
    
    gObjCalCharacter(aIndex);
    
    if(lpObj->IsFullSetItem != FALSE) lpObj->CharSet[11] |= 0x01;
    ...
    if(lpObj->pInventory[8].m_Type == ITEMGET(13,4)) lpObj->CharSet[12] |= 0x01; //Dark Horse
    ...
    if(lpObj->pInventory[8].m_Type == ITEMGET(13,37)){ //Fenrir
		...
		if((lpObj->pInventory[8].m_NewOption&4) == 4){ //Golden Fenrir
	...
	lpObj->pInventory[7].m_Type == ITEMGET(12,43)){
		lpObj->CharSet[5] |= 12;
		lpObj->CharSet[16] |= (((lpObj->pInventory[7].m_Type - 35) &7) * 4);
	...
	if(lpObj->pInventory[7].m_Type >= ITEMGET(12,130) && lpObj->pInventory[7].m_Type <= ITEMGET(12,134)){
		lpObj->CharSet[17] |= ((lpObj->pInventory[7].m_Type - 129) & 0x07) * 4; 
	}

	//New Wings (SEASON5 CUSTOMS)
	if(lpObj->pInventory[7].m_Type >= ITEMGET(12,180) && lpObj->pInventory[7].m_Type <= ITEMGET(12,183)){
		lpObj->CharSet[17] |= (((lpObj->pInventory[7].m_Type - 179) & 0x07) + 5) * 4; //24,28,32,36
	}
	
	switch(lpObj->pInventory[8].m_Type){

		case ITEMGET(13,64): //Hada
			lpObj->CharSet[16] |= 0x20;
		break;

		case ITEMGET(13,65): //Devil
			lpObj->CharSet[16] |= 0x40;
		break;

		case ITEMGET(13,67): //Rudolf
			lpObj->CharSet[16] |= 0x80;
		break;

		case ITEMGET(13,80): //Panda
...
void gObjStateSetCreate(int aIndex)
{
	...
	if(lpObj->RegenOk == 2 && lpObj->Type == OBJ_USER)
	{
		...
		if(Configs.TamaJJangEvent != 0 && Configs.AppearTamaJJang != 0)
		{
			if(lpObj->MapNumber == 0)
			{
				GCMapEventStateSend(0,Configs.AppearTamaJJang,2);
			}
		}
	}
		...
		if(lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if(lpObj->DieRegen == 2)
			{
				...
				if(State != 0)
				{
					...
					if(lpObj->Type == OBJ_USER)
					{
						if(Configs.UseCharacterAutoRecuperationSystem != 0 && lpObj->Level <= Configs.CharacterRecuperationMaxLevel)
						{
							lpObj->m_iAutoRecuperationTime = GetTickCount();
						}
						else
...
void gObjSecondProc()
{
	...
	for(n = 0; n < OBJMAX;n++)
	{
		...
		if(lpObj->Connected > PLAYER_LOGGED)
		{
			...
			if(lpObj->Type == OBJ_MONSTER)
			{
				if(lpObj->Class == 275)
				{
					gObjRefillMonsterHP(lpObj,Configs.KundunRefillHPSec);
				}
			...
			if(lpObj->Type == OBJ_USER)
			{
				...
				if(Configs.UseCharacterAutoRecuperationSystem != 0)
				{
					if(lpObj->Level <= Configs.CharacterRecuperationMaxLevel)
					{
		...
		if(lpObj->Connected > PLAYER_LOGGED &&	lpObj->Type == OBJ_USER)
		{
			...
			if(lpObj->CheckSumTime > 0 && GetTickCount() - lpObj->CheckSumTime > 5000 && Configs.DisableCheckSum == 0)
			{
	...
	if( Configs.TamaJJangEvent != 0)
	{
		Configs.TamaJJangTime--;

		if(Configs.TamaJJangTime <= 0)
		{
			if(Configs.AppearTamaJJang==0)
			{
				Configs.AppearTamaJJang = 1;
				Configs.TamaJJangTime = Configs.TamaJJangKeepTime;
				LogAddTD("Appear TamaJJang : Keep Time (%d)",Configs.TamaJJangTime);
			}
			else
			{
				Configs.AppearTamaJJang = 0;

				Configs.TamaJJangTime = Configs.TamaJJangDisappearTime +rand()% Configs.TamaJJangDisappearTimeRandomRange;
				LogAddTD("DisAppear TamaJJang : Keep Time (%d)",Configs.TamaJJangTime);
			}
...
void gObjViewportListProtocolCreate(LPOBJ lpObj)
{
	...
	else
	{
		...
		GensSystem.SendGensInfo(lpObj->m_Index);
		GensSystem.SendGensViewport(lpObj->m_Index);
...
void gObjViewportListProtocol(short aIndex)
{
	...
	else
	{
		...
		if(lpObj->Type == OBJ_USER)
		{
			...
			GensSystem.SendGensViewport(aIndex);
		}
	}
}
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	if(lpObj->DuelRoom > -1){
		if(lpObj->MapNumber == 64 && DuelManager.IsSpectator(aIndex,lpObj->DuelRoom)){
			DuelManager.DuelRemoveRequest(aIndex);
		}
	}
	...
	if(movefail == 0)
	{
		if(gGateC.GetGate(gt,(short &)x,(short &)y,(BYTE &)mapNumber,(BYTE &)dir,(short &)level) != -1)
		{
			if(gObj[aIndex].MapNumber != mapNumber)
			{
				...
				if(sSvrCode != Configs.GameServerCode)
				{
					if(sSvrCode == -1)
					{
						LogAddC(2,"[MapServerMng] Map Server Move Fail : CheckMoveMapSvr() == -1 [%s][%s] (%d)",lpObj->AccountID,lpObj->Name,lpObj->m_Index);
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	if(lpObj->pInventory[target].m_Type == ITEMGET(13,37))
	{
		...
		if(rand()% 10000 < Configs.FenrirRepairRate)
		{
	...
	(lpObj->pInventory[target].m_Type < ITEMGET(12,180) || lpObj->pInventory[target].m_Type >=  ITEMGET(12,183)) &&
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	LogAddTD(lMsg.Get(558), lpObj->AccountID,lpObj->Name,lpObj->pInventory[target].GetName(), 
	...
	g_kItemSystemFor380.Is380OptionItem(&lpObj->pInventory[target]) == TRUE){
		if(_r < Configs.SoulSpecialRate){
			bSuccess = true;
		}
	} else {
		if(g_SocketOption.IsSocketOption(&lpObj->pInventory[target]) == TRUE){
			if(_r < Configs.SoulSocketRate){
				bSuccess = true;
			}
		} else if(_r < Configs.SoulRate){
			bSuccess = true;
		}
	}
...
void gObjSetExpPetItem(int aIndex, int exp)
{
	...
	exp *= Configs.DarkSpiritAddExperience;
...
void gObjShieldAutoRefill(LPOBJ lpObj)
{
	...
	if(Configs.ShieldSystemOn == 0)
	{
		return;
	}

	if(Configs.ShieldAutoRefillOn == 0)
	{
		return;
	}

	if(Configs.ShieldAutoRefillOnSafeZone == 1)
	{
...
BOOL gCheckSkillDistance(int aIndex, int aTargetIndex, int iSkillNum)
{
	...
	iSkillDistance += Configs.SkillDistanceCheckTemp;
	LPOBJ lpObj = &gObj[aIndex];
...
void GCServerMsgStringSendAll(char* Message,int Type){
	for(int X=OBJ_STARTUSERINDEX; X < (OBJMAX-1);X++){
		if(gObjIsConnected(X)){
			GCServerMsgStringSend(Message,X,Type);
		}
	}
}