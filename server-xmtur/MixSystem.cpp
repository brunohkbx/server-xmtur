...
BOOL CMixSystem::DevilSquareEventChaosMix(LPOBJ lpObj, BOOL bCheckType, int iItemLevel)
{
	...
	switch ( iItemLevel )
	{
		...
		case 1:
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel1;
			nChaosNeedMoney = 100000;
			break;

		case 2:
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel2;
			nChaosNeedMoney = 200000;
			break;

		case 3:
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel3;
			nChaosNeedMoney = 400000;
			break;

		case 4:
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel4;
			nChaosNeedMoney = 700000;
			break;

		case 5:
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel5;
			nChaosNeedMoney = 1100000;
			break;

		case 6:
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel6;
			nChaosNeedMoney = 1600000;
			break;

		case 7: //Master
			lpObj->ChaosSuccessRate = Configs.DQChaosSuccessRateLevel7;
	...
	if ( g_CrywolfSync.GetOccupationState() == 0 && Configs.CrywolfApplyMvpBenefit )
	{	
...
BOOL CMixSystem::PlusItemLevelChaosMix(LPOBJ lpObj, int mixType)
{
	...
	if ( lpObj->pChaosBox[PlusItemPos].m_Level == 9 )
	{
		...
		{
			lpObj->ChaosSuccessRate = Configs.Items10MixRate;
		}
		else if(btMixLevel == 2) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items10SpecialRate;
		}
		else if(btMixLevel == 3) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items10WingsRate;
		}
	}
	else if ( lpObj->pChaosBox[PlusItemPos].m_Level == 10 || lpObj->pChaosBox[PlusItemPos].m_Level == 11) //season4 changed
	{
		if(btMixLevel == 1) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items11MixRate;
		}
		else if(btMixLevel == 2) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items11SpecialRate;
		}
		else if(btMixLevel == 3) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items11WingsRate;
		}
	}
	else if ( lpObj->pChaosBox[PlusItemPos].m_Level == 12) //season4 changed
	{
		if(btMixLevel == 1) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items13MixRate;
		}
		else if(btMixLevel == 2) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items13SpecialRate;
		}
		else if(btMixLevel == 3) //season4.5 add-on
		{
			lpObj->ChaosSuccessRate = Configs.Items13WingsRate;
		}
	}
	else if ( lpObj->pChaosBox[PlusItemPos].m_Level == 13) //season4 changed
	{
		
		lpObj->ChaosSuccessRate = Configs.Items14MixRate;
	
	}
	else if ( lpObj->pChaosBox[PlusItemPos].m_Level == 14) //season4 changed
	{
		
		lpObj->ChaosSuccessRate = Configs.Items15MixRate;
	}
	else
	{
...
void CMixSystem::BlessPotionChaosMix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = Configs.BlessPotionMixRate;

	int nChaosNeedMoney = Configs.BlessPotionMixCost;
...
void CMixSystem::CastleSpecialItemMix(LPOBJ lpObj) //Identical
{
	...
	if( Configs.CastleItemMixLimit <= 0 && Configs.RemoveCastleMixLimit == 0)
	{
...
void CMixSystem::Fenrir_01Level_Mix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = Configs.Fenrir_01Level_MixRate;
	lpObj->ChaosSuccessRate += iCharmOfLuckCount;
	int nChaosNeedMoney = 0;
...
void CMixSystem::Fenrir_02Level_Mix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = Configs.Fenrir_02Level_MixRate;
	lpObj->ChaosSuccessRate += iCharmOfLuckCount;
	int nChaosNeedMoney = 0;
...
void CMixSystem::Fenrir_03Level_Mix(LPOBJ lpObj)
{
	...
	lpObj->ChaosSuccessRate = Configs.Fenrir_03Level_MixRate;
	lpObj->ChaosSuccessRate += iCharmOfLuckCount;
...
void CMixSystem::ShieldPotionLv1_Mix(LPOBJ lpObj)
{
	...
	if (Configs.ShieldSystemOn == FALSE )
		return;
	...
	iChaosMixPrice = Configs.ShieldPotionLv1MixMoney;
	...
	if ( iRate < Configs.ShieldPotionLv1MixSuccessRate )
	{
	...
	else
	{
		...
		LogAddTD("[ChaosMix][Shield Potion] Lv1 Potion Mix [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d",
			lpObj->AccountID, lpObj->Name, Configs.ShieldPotionLv1MixSuccessRate,
...
void CMixSystem::ShieldPotionLv2_Mix(LPOBJ lpObj)
{
	...
	iChaosMixPrice = Configs.ShieldPotionLv2MixMoney;
	int iChaosTaxMoney = iChaosMixPrice * g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / 100;
	...
	if ( iRate < Configs.ShieldPotionLv2MixSuccessRate )
	{
	...
	else
	{
		...
		LogAddTD("[ChaosMix][Shield Potion] Lv2 Potion Mix [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d",
			lpObj->AccountID, lpObj->Name, 	Configs.ShieldPotionLv2MixSuccessRate,
			lpObj->Money, iChaosMixPrice, iCharmOfLuckCount);
	}
...
void CMixSystem::ShieldPotionLv3_Mix(LPOBJ lpObj)
{
	...
	iChaosMixPrice = Configs.ShieldPotionLv3MixMoney;
	int iChaosTaxMoney = iChaosMixPrice * g_CastleSiegeSync.GetTaxRateChaos(lpObj->m_Index) / 100;
	...
	if ( iRate < Configs.ShieldPotionLv3MixSuccessRate )
	{
	...
	else
	{
		...
		LogAddTD("[ChaosMix][Shield Potion] Lv3 Potion Mix [%s][%s] CBMix Fail %d Money : %d-%d, CharmRate : %d",
			lpObj->AccountID, lpObj->Name, 	Configs.ShieldPotionLv3MixSuccessRate,
			lpObj->Money, iChaosMixPrice, iCharmOfLuckCount);
	}
...
BOOL CMixSystem::CreateSemicronBook(LPOBJ gObj){
	...
	for(int X=0; X < 32;X++)
	{
		if(gObj->pChaosBox[X].m_Type == 0x1C67){
			PeaceOfBook_1++;
		} else if(gObj->pChaosBox[X].m_Type == 0x1C68){
			PeaceOfBook_2++;
		} else if(gObj->pChaosBox[X].m_Type == 0x1C69){
			PeaceOfBook_3++;
		} else if(gObj->pChaosBox[X].m_Type == 0x1C6A){
			PeaceOfBook_4++;
		} else if(gObj->pChaosBox[X].m_Type == 0x1C6B){
			PeaceOfBook_5++;
		} else if(gObj->pChaosBox[X].m_Type == 0x1C6C){
			PeaceOfBook_6++;
		}
	}
	...
	if(PeaceOfBook_1 < 1 || PeaceOfBook_2 < 1 || PeaceOfBook_3 < 1 
		&& PeaceOfBook_4 < 1 || PeaceOfBook_5 < 1 || PeaceOfBook_6 < 1){
		LogAddTD("[BookOfSecromicon_Mix][%s][%s] CBMix Fail... Some Requirements was Wrong",gObj->AccountID, gObj->Name);
		
		pMsg.Result = 0xF0;
		DataSend(gObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);

		gObj->ChaosLock = FALSE;
		return FALSE;
	}

	gObj->ChaosMoney = 1000000;
	gObj->ChaosLock = TRUE;
	LogAddTD("[BookOfSecromicon_Mix] Chaos Mix Start");

	//Set Money and Check
	if(gObj->ChaosMoney < 0){
		gObj->ChaosMoney = 0;
	}

	//Check Money
	if(gObj->Money < gObj->ChaosMoney)
	{
		LogAddTD("[BookOfSecromicon_Mix][%s][%s] CBMix Not Enough Money [%d] Need Zen [%d]",gObj->AccountID, gObj->Name, gObj->Money, gObj->ChaosMoney);
		/*pResult.Result = 2;
		DataSend(gObj->m_Index, (BYTE *)&pMsg, pMsg.Size);*/
		gObj->ChaosLock = FALSE;
		return TRUE;
	}

	gObj->Money -= gObj->ChaosMoney;
	GCMoneySend(gObj->m_Index, gObj->Money);

	gObj->ChaosSuccessRate = 80;

	if(gObj->ChaosSuccessRate > 100){
		gObj->ChaosSuccessRate = 100;
	}

	if((rand()%100) < gObj->ChaosSuccessRate){

		pMsg.Result = 1; //Success!

		::ItemSerialCreateSend(gObj->m_Index, -1, 0, 0, ITEMGET(14,109), 0, 255, 1, 0, 0, -1, 0, 0);
		::gObjInventoryCommit(gObj->m_Index);
		LogAddTD("[BookOfSecromicon_Mix][%s][%s] CBMix Success %d Money : %d-%d",gObj->AccountID, gObj->Name, gObj->ChaosSuccessRate, gObj->Money,gObj->ChaosMoney);
		return TRUE;
	} else {
		this->ChaosBoxInit(gObj);	
		GCUserChaosBoxSend(gObj, 0);
		DataSend(gObj->m_Index, (LPBYTE)&pMsg, pMsg.h.size);
		LogAddTD("[BookOfSecromicon_Mix][%s][%s] CBMix Fail %d Money : %d-%d",gObj->AccountID, gObj->Name, gObj->ChaosSuccessRate, gObj->Money,gObj->ChaosMoney);
		gObj->ChaosLock = FALSE;
		return FALSE;
	}
...
BOOL CMixSystem::SealedBoxMix(LPOBJ gObj){

	int SilverSealedBox = 0;
	int GoldenSealedBox = 0;
	int SKeyBox = 0;
	int GKeyBox = 0;

	for(int X=0; X < 32;X++){
		if(gObj->pChaosBox[X].m_Type == ITEMGET(14,121)){
			GoldenSealedBox += 1;
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(14,122)){
			SilverSealedBox += 1;
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(14,112)){
			SKeyBox += 1;
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(14,113)){
			GKeyBox += 1;
		}
	}

	PMSG_CHAOSMIXRESULT pResult;
	PHeadSetB((LPBYTE)&pResult.h, 0x86, sizeof(PMSG_CHAOSMIXRESULT));
	pResult.Result = CB_ERROR;

	if (SilverSealedBox > 1 || SKeyBox >1 || GoldenSealedBox > 1 || GKeyBox >1){
		LogAddTD("[S5_SealedBox][%s][%s] Key[%d/%d] Box[%d/%d] Overflow",gObj->AccountID,gObj->Name,SKeyBox,GKeyBox,SilverSealedBox,GoldenSealedBox);
		
		pResult.Result = 0xF0;
		DataSend(gObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
		gObj->ChaosLock = FALSE;
		return false;
	}

	gObj->ChaosLock = TRUE;
	LogAddTD("[S5_SealedBox] Chaos Mix Start");

	gObj->ChaosSuccessRate = 100;
	
	int Item;
	if(SilverSealedBox == 1){
		Item = ITEMGET(14,124); //Silver Box
	} else { 
		Item = ITEMGET(14,123); //Golden Box
	}

	::ItemSerialCreateSend(gObj->m_Index, -1, 0, 0,Item, 0, 255, 1, 0, 0, -1, 0, 0);
	::gObjInventoryCommit(gObj->m_Index);
	LogAddTD("[S5_SealedBox][%s][%s] CBMix Success %d Money : %d-%d",gObj->AccountID, gObj->Name, gObj->ChaosSuccessRate, gObj->Money,gObj->ChaosMoney);
	LogAddTD("[S5_SealedBox] Mix Finished");
	return TRUE;
}
BOOL CMixSystem::Extreme_CustomWingMixes(LPOBJ gObj){
	...
	for(int X=0; X < 32;X++){
		...
		if(gObj->pChaosBox[X].m_Type == ITEMGET(12,30) && gObj->pChaosBox[X].m_Level == 2){
			PackBless++;
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(12,31) && gObj->pChaosBox[X].m_Level == 2){
			PackSoul++; 
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(12,15)){
			Chaos++;
		} else if(gObj->pChaosBox[X].m_Type >= ITEMGET(12,37) && gObj->pChaosBox[X].m_Type <= ITEMGET(12,40) || gObj->pChaosBox[X].m_Type == ITEMGET(12,43)){
			ThirdWings++;
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(12,179)){
		CustomItem++;
		} else if(gObj->pChaosBox[X].m_Type == ITEMGET(14,22)){
			Creation++;
		}
	}
	...
	if(PackBless < 1 || PackSoul < 1|| Chaos < 1 || ThirdWings <1 || CustomItem < 1 || CustomItem < 1 || Creation < 1)
	{
		LogAddTD("[S5_ExtremeWings][%s][%s] CBMix Fail... Some Requirements was Wrong",gObj->AccountID, gObj->Name);
	...
	gObj->ChaosMoney = 10000000;
	gObj->ChaosLock = TRUE;
	LogAddTD("[S5_ExtremeWings] Chaos Mix Start");

	//Check Money
	if(gObj->Money < gObj->ChaosMoney)
	{
		LogAddTD("[S5_ExtremeWings][%s][%s] CBMix Not Enough Money [%d] Need Zen [%d]",gObj->AccountID, gObj->Name, gObj->Money, gObj->ChaosMoney);
		pResult.Result = CB_NOT_ENOUGH_ZEN;
		DataSend(gObj->m_Index, (BYTE *)&pResult, pResult.h.size);
		gObj->ChaosLock = FALSE;
	...
	if((rand()%100) < gObj->ChaosSuccessRate){
		...
		int RandomWings = rand()%3;
		int WingType = ITEMGET(12,180) + RandomWings;
		...
		int OptionRate = rand()%1000;
		int Option = 0;

		switch(rand()%4){
			case 1:
				if(OptionRate < 120) Option = 1;
			break;

			case 2:
				if(OptionRate < 60) Option = 2;
			break;

			case 3:
				if(OptionRate < 30) Option = 3;
			break;
		}

		
		int Excellent = 0;
		int WingsEffectRate = rand()%1000;

		switch(rand()%2){
			case 0:
				if(WingsEffectRate < 400) Excellent |= 0x10;
			break;

			case 1:
				if(WingsEffectRate < 300) Excellent |= 0x20;
			break;
		}

		int ExcellentRate = rand()%1000;
		switch(rand()%4){
			case 0:
				if(ExcellentRate < 40) Excellent |= 1;
			break;

			case 1:
				if(ExcellentRate < 20) Excellent |= 2;
			break;

			case 2:
				if(ExcellentRate < 70) Excellent |= 4;
			break;

			case 3:
				if(ExcellentRate < 70) Excellent |= 8;
			break;
		}
		::ItemSerialCreateSend(gObj->m_Index, -1, 0, 0, WingType , 0, 0, 0, Luck, Option, -1, Excellent, 0);
		::gObjInventoryCommit(gObj->m_Index);

		LogAddTD("[S5_ExtremeWings][%s][%s] CBMix Success %d Money : %d-%d",gObj->AccountID, gObj->Name, gObj->ChaosSuccessRate, gObj->Money,gObj->ChaosMoney);
		return true;
	} else {
		this->ChaosBoxInit(gObj);
		GCUserChaosBoxSend(gObj, 0);
		...
		DataSend(gObj->m_Index, (BYTE*)&pResult, pResult.h.size);
		LogAddTD("[S5_ExtremeWings][%s][%s] CBMix Fail %d Money : %d-%d",gObj->AccountID, gObj->Name, gObj->ChaosSuccessRate, gObj->Money,gObj->ChaosMoney);
		gObj->ChaosLock = FALSE;
		return false;
	}
}