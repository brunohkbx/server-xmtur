...
void GJPUserClose(LPSTR szAccountID)
{
	...
	cJoinServer.DataSend((char*)&pClose, pClose.h.size);
}
...
void JGPAccountRequest(SDHP_IDPASSRESULT * lpMsg)
{
	...
	{
		if ( Configs.NonPK == 0 && Configs.Language == 0 )
		{
		...
		if ( Configs.PkLimitFree == 1 && Configs.Language == 0 )
		{
...
void JGPEventControl( SDHP_EVENTCONTROL * lpMsg )
{
	switch ( lpMsg->Event  )
	{
		...
		case XMAS_EVENT:
			Configs.XMasEvent = lpMsg->Stat;
			break;

		case FIRECRACKER_EVENT:
			Configs.FireCrackerEvent = lpMsg->Stat;
			break;

		case HEARTOFLOVE_EVENT:
			Configs.HeartOfLoveEvent = lpMsg->Stat;
			break;

		case MEDAL_EVENT:
			Configs.MedalEvent = lpMsg->Stat;
			break;
	}
}
...
void GJReqMapSvrMove(int iIndex, WORD wDesMapSvrCode, WORD wMapNumber, BYTE btX, BYTE btY)
{
	...
	else
	{
		...
		pMsg.wCurMapSvrCode = Configs.GameServerCode;
...
void GJReqMapSvrAuth(int iIndex, LPSTR lpszAccountID, LPSTR lpszCharName, int iJA1, int iJA2, int iJA3, int iJA4)
{
	...
	pMsg.wDstMapSvrCode = Configs.GameServerCode;
...
void GJNotifyMaxUserCount()
{
	...
	pMsg.iSvrCode = Configs.GameServerCode;
