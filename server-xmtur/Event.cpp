...
void DataSendEventChip(char* pMsg, int size)
{
	...
	{
		cEventServer.Close();
		cEventServer.CreateSocket(ghWnd);
	...
	if ( ::IsEventChipServerConnected != FALSE && ::EventChipServerConnect != FALSE )
	{
		::cEventServer.DataSend((char *)pMsg, size);
	}
}
...
void EventChipOpenEven(LPOBJ lpObj)
{
	...
	if ( (rand()%100) < ::Configs.EventChipDropRateForBoxOfGold )
	{
		dur = 255.0f;
		x = lpObj->X;
		y = lpObj->Y;
	...
	{
		if ( (rand()%20) < ::Configs.ItemDropRateForBoxOfGold )
		{
...
void GoldMedalOpenEven(LPOBJ lpObj)
{
	...
	{
		if ( (rand()%20) < Configs.ItemDropRateForGoldMedal )
		{
...
void SilverMedalOpenEven(LPOBJ lpObj)
{
	...
	{
		if ( (rand()%20) < Configs.ItemDropRateForSilverMedal )
		{
...
void HeartOfLoveOpenEven(LPOBJ lpObj)
{
	...
	{
		if ( (rand()%20) < Configs.ItemDropRateForgHeartOfLove )
		{
...
void FireCrackerOpenEven(LPOBJ lpObj)
{
	...
	if ( Configs.OnlyFireCrackerEffectUse )
	{
		PMSG_SERVERCMD ServerCmd;
	...
	{
		if ( (rand()%10) < Configs.ItemDropRateForgFireCracker )
		{
...
void DarkLordHeartItemBoxOpen(LPOBJ lpObj)
{
	...
	if ( ((DWORD)rand()%60000) < (DWORD)Configs.DarkLordHeartOffEventRate )	
	{
		if ( lpObj->Level > 100 )
		{
			return;
...
void EGReqBloodCastleEnterCount(int iIndex)
{
	...
	pMsg.ServerCode = Configs.GameServerCode / 20;
...
void EGReqRegCCOfflineGift(int iIndex)
{
	...
	pMsg.wServerCode = Configs.GameServerCode / 20;
	pMsg.iINDEX = iIndex;
	pMsg.szUID[10] = 0;
	pMsg.szNAME[10] = 0;
	...
	case 0:
		{
			if(lpMsg->w_14 == 1)
			{
				...
				Configs.XMasVisitCount++;
				wsprintf(szText,lMsg.Get(MSGGET(10,24)),Configs.XMasVisitCount);
				
				if ( (Configs.XMasVisitCount % Configs.XMasEvent_LuckNumber1st) == FALSE )
				{
					if ( (Configs.XMasVisitCount % Configs.XMasEvent_LuckNumber2nd) == FALSE )
					{
				...
				if((Configs.XMasVisitCount % 100) != 0 && Configs.NewServer != 1)
				{
					XMasEventA->DropItem(aIndex,lpObj->MapNumber,lpObj->X, lpObj->Y);
				}
				
				LogAddTD("[X-MAX Event] [AccountID]: %s , [VISITCOUNT]:%d",lpObj->AccountID,Configs.XMasVisitCount);
