...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic, int magicsend, BYTE MSBFlag, int AttackDamage, BOOL bCombo)
{
	...
	if(SafeMap.SafeCheck(lpObj->m_Index,lpTargetObj->m_Index) == FALSE){
		return FALSE;	
	}
	...
	if(lpTargetObj->Type == OBJ_MONSTER){
		...
		for(int X=0; X < ImperialGuardian.TotalDoors;X++){
			...
			if(lpTargetObj->m_Index == ImperialGuardian.Door[X]){
				...
				if(ImperialGuardian.BattleStatus == IMPERIAL_STANDBY || ImperialGuardian.BattleStatus == IMPERIAL_NONE){
					return FALSE;
				}

				if(ImperialGuardian.CurrentMonsters > 0){
					if(ImperialGuardian.Step == 1 || ImperialGuardian.Step == 4 || ImperialGuardian.Step == 7){
						return FALSE;
					}
				}
			}
	...
	if ( Configs.UseCharacterAutoRecuperationSystem && lpObj->Level <= Configs.CharacterRecuperationMaxLevel )
	{
		...
		{
			if ( lpTargetObj->Level <= Configs.CharacterRecuperationMaxLevel )
			{
	...
	if ( AttackDamage == 0 )
	{
		if (Configs.ShieldSystemOn == TRUE )
		{
		...
		if ( skill == 19
			...
			|| (g_MasterSkillSystem.GetSkillID(skill) == 540)
			|| (g_MasterSkillSystem.GetSkillID(skill) == 545)
			|| (g_MasterSkillSystem.GetSkillID(skill) == 550)
			|| skill == 235
			|| skill == 232
			|| skill == 236
			|| skill == 238)
		...
		if(Configs.AbilityDebug == 1 && (MsgDamage == 3 || MsgDamage == 2)) //Season 3 Add-on for Ability Debug
		{
			char szTemp[256];
		...
		if ( lpTargetObj->Live )
		{
			switch ( skill )
			{
				...
				case 260: //Segun esto, utliza Vitalidad.. ? o.O
                case 261:
                    AttackDamage = AttackDamage * (double)(Vitality / 10 + 50) / 100.0;
				break;

				case 262:
					AttackDamage = (double)(Vitality / 10 + AttackDamage) * (double)(Vitality / 10 + 50) / 100.0;
				break;

                case 263:
                    AttackDamage = (double)(Energy / 8 + AttackDamage + Dexterity / 10) * (double)(Energy / 8 + Dexterity / 10 + 100) / 100.0;
				break;

				case 264:
                    AttackDamage = (double)(Dexterity / 10 + AttackDamage) * (double)(Dexterity / 10 + 50) / 100.0;
                break;

                case 265:
                    AttackDamage = (double)AttackDamage * (double)(Dexterity / 10 + 50) / 100.0;
				break;

                case 270:
                    AttackDamage = (double)AttackDamage * (double)(Vitality / 10 + 50) / 100.0;
				break;

                case 269:
                    AttackDamage = AttackDamage * (Vitality / 10 + 200) / 100;
				break;
		...
		if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
		{
			if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER )
			{
				if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE && lpTargetObj->MapNumber == MAP_INDEX_CASTLESIEGE )
				{
					...
					else if(Configs.ShieldSystemOn == 0)
					{
						AttackDamage = AttackDamage * 40 / 100;
					}
				}
	...
	if ( selfdefense == TRUE && bDamageReflect == FALSE )
	{
		...
		{
			if(lpCallObj->PartyNumber >= 0 && Configs.FixRemovePKPartySystem == 0) //Season 2.5 add-on
			{
				int number = 0;
...
int CObjAttack::GetShieldDamage(LPOBJ lpObj, LPOBJ lpTargetObj, int iAttackDamage)
{
	...
	int iDamageDevideToSDRate = Configs.DamageDevideToSDRate;
