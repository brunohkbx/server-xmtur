...
int CCrywolfUtil::CrywolfMVPLevelUp(int iUserIndex, int iAddExp)
{
	...
	else
	{
		...
		{
			gObj[iUserIndex].LevelUpPoint += Configs.CWDLMGPoints;
		}
		else
		{
			gObj[iUserIndex].LevelUpPoint += Configs.CWNormalPoints;
		}

		if ( gObj[iUserIndex].PlusStatQuestClear != false )
		{
			gObj[iUserIndex].LevelUpPoint += Configs.CWMarlonPoints;
