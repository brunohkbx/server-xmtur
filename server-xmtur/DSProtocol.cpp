...
void DataServerProtocolCore(BYTE protoNum, BYTE *aRecv, int aLen)
{
	switch ( protoNum )
	{
		...
		//Season5 Quest
		case 0xB5:
			DGQuestManagerList((PMSG_ANS_QUESTLIST_MANAGER *)aRecv);
		break;
		...
		case 0xB9:{
			...
			switch(lpMsg->subcode){
				case 0x01:
					DGSetGensInfo((PMSG_ANS_GENS_INFO*)aRecv);
				break;

				case 0x04:
					DGJoinGensAnswer((PMSG_ENTER_ANS_GENS_INFO*)aRecv);
				break;
			}
		}
		break;
		...
		case 0xA8:{
			...
			switch(lpMsg->subcode){
				case 0x01:
					DGGetExtremePoints((PMSG_ANS_EXTREMEPOINTS*)aRecv);
				break;
			}
		}
		break;
		...
		case 0xA9:{
			...
			switch(lpMsg->subcode){
				case 0x01:
					DGGetMarryInfo((PMSG_SEND_MARRYINFO*)aRecv);
				break;
			}
		}
		break;
		...
		case 0xAA:{
			...
			switch(lpMsg->subcode){
				case 0x02:
					GetWarehouseInUse((PMSG_ANS_WAREHOUSEINUSE*)aRecv);
				break;
			}
		}
		break;
		...
		case 0xFF: {
			...
			LogAddC(2, "error-L2DataServer RECV : %d (%d) %02X %02X",aLen,pMsg->count,aRecv[2],aRecv[3]);
		}
		break;
	}
}
...
void GetWarehouseInUse(PMSG_ANS_WAREHOUSEINUSE* aRecv){
	...
	if(OBJMAX_RANGE(aRecv->aIndex) == FALSE){
		LogAddTD("[GetWarehouseInUse] Index out of Range (%d)",aRecv->aIndex);
		return;
	}

	LPOBJ lpObj = &gObj[aRecv->aIndex];
	lpObj->VaultInUse = aRecv->InUse;
	LogAddTD("[MultiVault] Warehouse in Use (%d)",lpObj->VaultInUse);
}

struct PMSG_REQ_WAREHOUSEINUSE {
	PBMSG_HEAD2 h;	// C1:B9
	int aIndex;	// 4
	char Account[11];	// C
};
...
void ReqWarehouseInUse(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[ReqWarehouseInUse] Index out of Range");
		return;
	}
	...
	LogAddTD("[MultiVault][%s][%s] Request MultiVault Info",lpObj->AccountID,lpObj->Name);
}

void DGGetMarryInfo(PMSG_SEND_MARRYINFO* aRecv){

	if(OBJMAX_RANGE(aRecv->aIndex) == FALSE){
		LogAddTD("[DGGetMarryInfo] Index out of Range (%d)",aRecv->aIndex);
		return;
	}
	...
	if(aRecv->Result == 1){
		...
		char Buff[255];
		memcpy(lpObj->HusbandWifeName,aRecv->HusbandWife,sizeof(lpObj->HusbandWifeName));
		...
		LogAddTD("[MarrySystem][%s][%s] Marry Info Seted",lpObj->AccountID,lpObj->Name);
	} else {
	
		LogAddTD("[MarrySystem][%s][%s] Not Info Found",lpObj->AccountID,lpObj->Name);
	}
}
...
void DSGetMarryInfo(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DSGetMarryInfo] Index out of Range");
		return;
	}
...
void DSSetMarryInfo(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DSSetMarryInfo] Index out of Range");
		return;
	}
...
void DSDeleteMarryInfo(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DSDeleteMarryInfo] Index out of Range");
		return;
	}
	...
	LogAddTD("[MarrySystem][%s][%s] Delete Marry Info",lpObj->AccountID,lpObj->Name);
}

void DGGetExtremePoints(PMSG_ANS_EXTREMEPOINTS* aRecv){
	
	if(OBJMAX_RANGE(aRecv->aIndex) == FALSE){
		LogAddTD("[DGGetExtremePoints] Index out of Range (%d)",aRecv->aIndex);
		return;
	}

	LPOBJ lpObj = &gObj[aRecv->aIndex];

	lpObj->ExtremePoints = aRecv->Points;

	char Buff[255];
	wsprintf(Buff,"Tienes %d punto(s) Extreme.",lpObj->ExtremePoints);
	GCServerMsgStringSend(Buff,lpObj->m_Index,1);

	LogAddTD("[GetExtremePoints][%s][%s] Set Extreme Points",lpObj->AccountID,lpObj->Name,lpObj->ExtremePoints);
}

void DSGetExtremePoints(int aIndex){
	
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[GetExtremePoints] Index out of Range");
		return;
	}
...
void DSSaveExtremePoints(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DSSaveExtremePoints] Index out of Range");
		return;
	}
	
	LPOBJ lpObj = &gObj[aIndex];

	PMSG_SAVE_EXTREMEPOINTS pMsg;
...
void DGJoinGensRequest(int aIndex, int GensType){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DGJoinGensRequest] Index out of Range");
		return;
	}

	PMSG_ENTER_GENS_INFO pMsg;
	pMsg.h.set((LPBYTE)&pMsg,0xB9,0x04,sizeof(pMsg));
...
void DGJoinGensAnswer(PMSG_ENTER_ANS_GENS_INFO* aRecv){
	...
	if(OBJMAX_RANGE(aRecv->aIndex) == FALSE){
		LogAddTD("[DGSetGensInfo] Index out of Range");
		return;
	}
	...
	if(aRecv->Result == 0x07){
		pMsg.Type = 0x07;
		pMsg.Sub = 0x00;
		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.Size);
		LogAddTD("[GensSystem][%s][%s] Guild is part of a Guild Alliance",gObj->AccountID,gObj->Name);
		return;
	}

	if(aRecv->Result == 0x05){
		pMsg.Type = 0x05;
		pMsg.Sub = 0x00;
		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.Size);
		LogAddTD("[GensSystem][%s][%s] Guild Master is not in Family",gObj->AccountID,gObj->Name);
		return;
	}

	if(aRecv->Result == 0x04){
		pMsg.Type = 0x04;
		pMsg.Sub = 0x00;
		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.Size);
		LogAddTD("[GensSystem][%s][%s] Guild Master is in Other Gens Family",gObj->AccountID,gObj->Name);
		return;
	}

	if(aRecv->Result == 0x00){
		//Join Gens
		pMsg.Type = 0x00;
		pMsg.Sub = 0x00;

		lpObj->GensFamily = aRecv->GensType;
		lpObj->GensRank = 0;
		lpObj->GensContribution = 0;

		DataSend(lpObj->m_Index,(LPBYTE)&pMsg,pMsg.Size);
		GensSystem.SendGensInfo(lpObj->m_Index);
		LogAddTD("[GensSystem][%s][%s] Joined Gens Successfully! (GensFamily: %d)",lpObj->AccountID,lpObj->Name,aRecv->GensType);
	}
}
...
void DGSetGensInfo(PMSG_ANS_GENS_INFO* aRecv){
	...
	if(aRecv->Result == 1){
		lpObj->GensFamily = aRecv->Type;
		lpObj->GensContribution = aRecv->Contribution;
		lpObj->GensRank = aRecv->Rank;
		lpObj->GensPrize = aRecv->Prize;
		GensSystem.SendGensInfo(lpObj->m_Index);
		LogAddTD("[GensSystem][%s][%s] Send Gens Information",lpObj->AccountID,lpObj->Name);
	} else {
		lpObj->GensFamily = 0;
		lpObj->GensContribution = 0;
		lpObj->GensRank = 0;
...
void DGUpdateGensInfo(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DGUpdateGensInfo] Index out of Range");
		return;
	}

	PMSG_SAVE_GENS_INFO pMsg;
	pMsg.h.set((LPBYTE)&pMsg,0xB9,0x02,sizeof(pMsg));

	LPOBJ lpObj = &gObj[aIndex];

	if(lpObj->GensFamily > 0){
		pMsg.Type = lpObj->GensFamily;
...
void DGDeleteGensInfo(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DGDeleteGensInfo] Index out of Range");
		return;
	}

	LPOBJ lpObj = &gObj[aIndex];

	PMSG_DELETE_GENS_INFO pMsg;
...
void DGGetGensInfo(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DGGetGensInfo] Index out of Range");
		return;
	}

	LPOBJ lpObj = &gObj[aIndex];

	PMSG_REQ_GENS_INFO pMsg;
...
void DGUpdateGensRanking(){
	...
	BYTE pMsg[4] = { 0xC1,0x04,0xB9,0x05 };
	LogAddTD("[GensSystem] Update Gens Ranking!");
	
	cDBSMng.Send((char*)&pMsg,4);
}

///////////////////////////////////////////////////////////

struct PMSG_REQ_QUESTLIST_MANAGER {
	PBMSG_HEAD h;	// C1:B5
	int aIndex;	// 4
	char Name[11];	// C
};
...
void JGPQuestManagerList(int aIndex){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[JGPQuestManagerList] Index out of Range");
		return;
	}
	...
	PMSG_REQ_QUESTLIST_MANAGER pMsg;
...
struct PMSG_REQ_QUESTLIST_DELETE {
	PBMSG_HEAD h;	// C1:B5
	int aIndex;	// 4
	char Name[11];	// C
	int Type;
...
void DGDeleteQuestList(int aIndex, int Type, int Index){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DGDeleteQuestList] Index out of Range");
		return;
	}
	...
	PMSG_REQ_QUESTLIST_DELETE pMsg;
...
void DGInsertQuestList(int aIndex, int Type, int Index){
	...
	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAddTD("[DGInsertQuestList] Index out of Range");
		return;
	}
...
void DGQuestManagerList(PMSG_ANS_QUESTLIST_MANAGER * aRecv){

	BYTE Count = 0;

	if(OBJMAX_RANGE(aRecv->aIndex) == FALSE){
		LogAddTD("[DGQuestManagerList] Index out of Range");
		return;
	}
	...
	for(int i=0; i < MAX_ACCEPT_QUEST; i++){
		lpObj->QuestInfo[i].Index = (BYTE)-1;
		lpObj->QuestInfo[i].Type = (BYTE)-1;
		memset(lpObj->QuestInfo[i].Requirement,0,sizeof(lpObj->QuestInfo[i].Requirement));
	}

	for(int i=0; i < MAX_ACCEPT_QUEST;i++){
		if(aRecv->QuestInfo[i].Index != (BYTE)-1 && aRecv->QuestInfo[i].Type != (BYTE)-1){
			lpObj->QuestInfo[i].Index = aRecv->QuestInfo[i].Index; 
			lpObj->QuestInfo[i].Type = aRecv->QuestInfo[i].Type; 
			lpObj->QuestInfo[i].Requirement[0] = aRecv->QuestInfo[i].Requirement[0]; 
			lpObj->QuestInfo[i].Requirement[1] = aRecv->QuestInfo[i].Requirement[1]; 
			lpObj->QuestInfo[i].Requirement[2] = aRecv->QuestInfo[i].Requirement[2]; 
			lpObj->QuestInfo[i].Requirement[3] = aRecv->QuestInfo[i].Requirement[3]; 
			lpObj->QuestInfo[i].Requirement[4] = aRecv->QuestInfo[i].Requirement[4]; 
			Count++;
		}
	}

	QuestManager.SendQuestToPlayer(lpObj->m_Index);
}

void DGQuestManagerSaveList(int aIndex){

	BYTE Count = 0;

	if(OBJMAX_RANGE(aIndex) == FALSE){
		LogAdd("[DGQuestManagerSaveList] Index out of Range");
		return;
	}

	PMSG_ANS_QUESTLIST_MANAGER pMsg;
	
	pMsg.h.set((LPBYTE)&pMsg,0xB6,sizeof(pMsg));

	for(int i=0; i < MAX_ACCEPT_QUEST; i++){
		pMsg.QuestInfo[i].Index = (BYTE)-1;
		pMsg.QuestInfo[i].Type = (BYTE)-1;
		memset(pMsg.QuestInfo[i].Requirement,0,sizeof(pMsg.QuestInfo[i].Requirement));
	}

	LPOBJ lpObj = &gObj[aIndex];

	for(int i=0; i < MAX_ACCEPT_QUEST;i++){
		if(lpObj->QuestInfo[i].Type != (BYTE)-1 && lpObj->QuestInfo[i].Index != (BYTE)-1){
			pMsg.QuestInfo[i].Index = lpObj->QuestInfo[i].Index; 
			pMsg.QuestInfo[i].Type = lpObj->QuestInfo[i].Type; 
			pMsg.QuestInfo[i].Requirement[0] = lpObj->QuestInfo[i].Requirement[0]; 
			pMsg.QuestInfo[i].Requirement[1] = lpObj->QuestInfo[i].Requirement[1]; 
			pMsg.QuestInfo[i].Requirement[2] = lpObj->QuestInfo[i].Requirement[2]; 
			pMsg.QuestInfo[i].Requirement[3] = lpObj->QuestInfo[i].Requirement[3]; 
			pMsg.QuestInfo[i].Requirement[4] = lpObj->QuestInfo[i].Requirement[4]; 
			Count++;
		}
	}
	...
	LogAdd("[QuestManager][%s][%s] %d Quests Saved",lpObj->AccountID,lpObj->Name,Count);
}
...
void DataServerLogin(int server)
{
	...
	pInfo.Port =(WORD)Configs.GameServer_Port;
	pInfo.Type =1;
	pInfo.ServerCode = Configs.GameServerCode;
	...
	if ( pCLCount.Count > 0 )
	{
		for ( int n=0;n<pCLCount.Count ;n++)
		{
			...
			if(lpCL->DbVersion == 0)
			{
				...
				if(lpCL->dbInventory[4] == (BYTE)-1) pCList.CharSet[9] = 0x80;
				if(lpCL->dbInventory[6] == (BYTE)-1) pCList.CharSet[9] |= 0x40;
				if(lpCL->dbInventory[8] == (BYTE)-1) pCList.CharSet[9] |= 0x20;
				if(lpCL->dbInventory[10] == (BYTE)-1) pCList.CharSet[9] |= 0x10;
				if(lpCL->dbInventory[12] == (BYTE)-1) pCList.CharSet[9] |= 0x08;
				...
				if(lpCL->dbInventory[4] == (BYTE)-1) pCList.CharSet[13] |= 0x0F;
				if(lpCL->dbInventory[8] == (BYTE)-1) pCList.CharSet[14] |= 0xF0;
				if(lpCL->dbInventory[10] == (BYTE)-1) pCList.CharSet[14] |= 0x0F;
				if(lpCL->dbInventory[12] == (BYTE)-1) pCList.CharSet[15] |= 0xF0;
				if(lpCL->dbInventory[14] == (BYTE)-1) pCList.CharSet[15] |= 0x0F;
				...
				if(TempInventory[2] > 0x0F) pCList.CharSet[9] = 0x80;
				if(TempInventory[3] > 0x0F) pCList.CharSet[9] |= 0x40;
				if(TempInventory[4] > 0x0F) pCList.CharSet[9] |= 0x20;
				if(TempInventory[5] > 0x0F) pCList.CharSet[9] |= 0x10;
				if(TempInventory[6] > 0x0F) pCList.CharSet[9] |= 0x8;
				...
				if((TempInventory[7] >= 0x24 && TempInventory[7] <= 0x28)|| TempInventory[7] == 0x2B){
					pCList.CharSet[5] |= 0x0C;
					pCList.CharSet[16] |= (( TempInventory[7] - 0x23)& 0x07) << 2;
				...
				if((TempInventory[8] & 0x03) != 0 && TempInventory[8] != 0x1F) pCList.CharSet[10] |= 1;

				pCList.CharSet[11] = 0;

				if((TempInventory[8] & 0x04) != 0 && TempInventory[8] != 0x1F) pCList.CharSet[12] |= 1;
				...
				if((TempInventory[8] & 0x04) == 0){
					if(TempInventory[8] == 0x1FF){
						Index |= 0x03;
					} else if( TempInventory[8] != 0x43 && TempInventory[8] != 0x7B){
						Index |= (TempInventory[8])& 0x03;
					}
				} else {
					Index |= 0x03;
				...
				if((TempInventory[7] >= 3 && TempInventory[7] <= 6) || TempInventory[7] == 0x1E || 
				TempInventory[7] == 0x29 || TempInventory[7] == 0x2A){
					pCList.CharSet[5] |= 0x0C;
				...
				pCList.CharSet[10] = 0;
				if((TempInventory[8] & 0x03) != 0 && TempInventory[8] != 0x1FF) pCList.CharSet[10] |= 0x01;

				pCList.CharSet[11] = 0;
				if((TempInventory[8]& 0x04) != 0 && TempInventory[8] != 0x1FF) pCList.CharSet[12] |= 0x01;
				...
				if(TempInventory[8] == 0x25 && TempInventory[8] != 0x1FF){
					...
					if((btExcellentOption&1) == 1) pCList.CharSet[16] |= 0x01;
					
					if((btExcellentOption&2) == 2) pCList.CharSet[16] |= 0x02;
			
					if((btExcellentOption&4) == 4) pCList.CharSet[17] |= 0x01;
				}
			}
			...
			//Mini Wings
			if(TempInventory[7] >= 130 && TempInventory[7] <= 134){
				pCList.CharSet[17] |= ((TempInventory[7] - 129) & 0x07) * 4; //Original << 5.. THIS IS CUSTOM FOR NEW CUSTOM WINGS WORK!
			}

			//New Wings (SEASON5 CUSTOMS)
			if(TempInventory[7] >= 180 && TempInventory[7] <= 183){
				pCList.CharSet[17] |= (((TempInventory[7] - 179) & 0x07) + 5)  * 4; //24,28,32,36
			}
...
void JGGetCharacterInfo( SDHP_DBCHAR_INFORESULT * lpMsg)
{
	...
	if ( lpObj->m_bMapSvrMoveReq == false )
	{
		...
		if ( wGameServerCode != Configs.GameServerCode )
		{
	...
	if (Configs.TamaJJangEvent != FALSE )
	{
		if (Configs.AppearTamaJJang != 0 )
		{
			if ( lpObj->MapNumber == MAP_INDEX_RORENCIA )
			{
				GCMapEventStateSend(MAP_INDEX_RORENCIA, Configs.AppearTamaJJang, 2);
			}
		}
	}
	...
	for ( int n=0;n<INVENTORY_SIZE;n++)
	{
		if ( lpObj->pInventory[n].IsItem() == TRUE )
		{
			...
			if(lpObj->pInventory[n].m_ExpirationItem == TRUE){
				g_PropItems.SendPropertyInfo(lpObj, lpObj->pInventory[n].m_Type, lpObj->pInventory[n].m_Number, n);
			}
		}
	}
...
void GS_GDReqMapSvrMsgMultiCast( int iMapSvrGroup,char * lpszMsgText)
{
	...
	GCServerMsgStringSendAll(lpszMsgText,0);
...
void GDReqInGameShopItemList(int aIndex)
{
	...
	LogAddTD("[InGameShop][%s] Requesting Shop ItemList to DB", gObj[aIndex].AccountID);
}
...
void DGAnsInGameShopItemList(LPBYTE lpRecv)
{
	...
	ISHOP_ITEMLIST * ItemInfo = (ISHOP_ITEMLIST*)(lpRecv + sizeof(ISHOP_ANS_ITEMLIST));

	lsOfs += sizeof(ISHOP_ANS_ITEMLIST);

	LPOBJ lpObj = &gObj[lpMsg->aIndex];

	LogAddTD("[InGameShop][%s] ItemList Total Count %d",lpObj->AccountID,lpMsg->Count);

	for(int i=0;i< lpMsg->Count;++i){

		ItemInfo = (ISHOP_ITEMLIST*)(lpRecv + lsOfs);

		g_CashShop.InsertItemToInventory(lpObj, ItemInfo->UniqueCode, ItemInfo->AuthCode,
		ItemInfo->UniqueID1, ItemInfo->UniqueID2, ItemInfo->UniqueID3, ItemInfo->InventoryType);
		
		lsOfs += sizeof(ISHOP_ITEMLIST);
	}	
}
...
void GDReqInGameShopPoint(int aIndex)
{
	...
	cDBSMng.Send((PCHAR)&pMsg, sizeof(pMsg));
	LogAddTD("[InGameShop][%s] Requesting Point Info to DB",gObj[aIndex].AccountID);
}
...
void GDReqInGameShopItemListSave(int aIndex)
{
	...
	for(int i=0;i<30;++i)
	{
		Item = (ISHOP_ITEMLIST *)(Buff+sizeof(ISHOP_ANS_ITEMLIST)+Count*sizeof(ISHOP_ITEMLIST));
	...
	PHeadSubSetW((LPBYTE)Info,0xD2,0x05,PacketSize);
