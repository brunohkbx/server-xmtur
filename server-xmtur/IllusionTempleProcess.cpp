...
void CIllusionTempleProcess::ProcState_Closed(int iTick) 
{
	...
	if(this->m_iLeftMinutesBeforeOpen <= 0)
	{
		if(this->m_iEventBattleTimer <= 0)
		{
			...
			if(this->m_iEnteredUserCount < Configs.IT_MinPlayers) //UserCount
			{
	...
	if(g_iSaveIllusionTempleRankingPoint != FALSE)
	{
		for(i = 0;i < MAX_FLOOR_USER;i++)
		{
			if(this->m_FloorData[i].m_iTeamPlayerIndex != -1 && gObj[this->m_FloorData[i].m_iTeamPlayerIndex].Connected > PLAYER_LOGGED)
			{
				...
				RankMsg.ServerCode = Configs.GameServerCode / 20;
...
void CIllusionTempleProcess::DropRewardItem(int aIndex)
{
	...
	int loc2 = aIndex;
	int loc3 = ItemGetNumberMake(Configs.IT_Group, Configs.IT_ID);	// Jewel of Chaos
	ItemSerialCreateSend(gObj[aIndex].m_Index, gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y,loc3, 
		Configs.IT_Level, Configs.IT_Dur, Configs.IT_Luck, Configs.IT_Skill, Configs.IT_Opt, loc2, Configs.IT_Exc, 0);
}
...
int CIllusionTempleProcess::LevelUp(int iUserIndex, int iAddExp) 
{
	...
	else
	{
		...
		{
			gObj[iUserIndex].LevelUpPoint += Configs.ITDLMGPoints;
		}
		else
		{
			gObj[iUserIndex].LevelUpPoint += Configs.ITNormalPoints;
		}

		if(gObj[iUserIndex].PlusStatQuestClear != false)
		{
			gObj[iUserIndex].LevelUpPoint += Configs.ITMarlonPoints;
