...
void CMonsterItemMng::gObjGiveItemSearch(int monsterlevel, int maxlevel)
{
	...
	while ( true )
	{
			...
			(type == 14 && index == 31 ) )
		{
			...
			if ( type == 14 && index == 17 )
			{
				if ( Configs.EyesOfDevilSquareDropRate <= 0 )
				{
					Configs.EyesOfDevilSquareDropRate = 1;
				}

				perc = rand() % Configs.EyesOfDevilSquareDropRate;
				bCheckDevil = 1;
			...
			if ( type == 14 && index == 18 )
			{
				perc = rand() % Configs.KeyOfDevilSquareDropRate;
				bCheckDevil = 1;
