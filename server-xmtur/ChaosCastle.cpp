...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	{
		std::vector<CHAOSCASTLE_START_TIME>::iterator stCCTime = it;
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex) ==TRUE && gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (this->GetChaosCastleMapNumber(iChaosCastleIndex)) )
			{
				...
				if ( btMapAttr == 8 )
				{
					...
					if (iUSER_AXIS == MAX_CC_TRAP_STEP )
						continue;
...
void CChaosCastle::SetItemsToMonster(int iChaosCastleIndex) //005A4940
{
	...
	if(Configs.NewServer == TRUE) //season4.5 add-on
	{
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		if ( gObj[iUserIndex].Class == CLASS_DARKLORD || gObj[iUserIndex].Class == CLASS_MAGUMSA )
		{
			gObj[iUserIndex].LevelUpPoint += Configs.CCDLMGPoints;
		}
		else
		{
			gObj[iUserIndex].LevelUpPoint += Configs.CCNormalPoints;
		}

		if ( gObj[iUserIndex].PlusStatQuestClear != false )
		{
			gObj[iUserIndex].LevelUpPoint += Configs.CCMarlonPoints;
...
bool CChaosCastle::CheckCanStartPlay(int iChaosCastleIndex)
{
	...
	if ( iEnteredUser < Configs.CCMinPlayers )
	{
...
BOOL CChaosCastle::CheckWearingMOPH(int iUserIndex)
{
	...
	if ( gObj[iUserIndex].pInventory[10].IsItem()  )
	{
		...
		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,122) ) //season 4.0 add-on
		{
			return TRUE;
		}
		if ( gObj[iUserIndex].pInventory[10].m_Type == ITEMGET(13,76) ) //season 4.0 add-on
		{
			return TRUE;
		}
	}
	...
	if ( gObj[iUserIndex].pInventory[11].IsItem() )
	{
		...
		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,122) ) //season 4.0 add-on
		{
			return TRUE;
		}
		if ( gObj[iUserIndex].pInventory[11].m_Type == ITEMGET(13,76) ) //season 4.0 add-on
		{
			return TRUE;
		}
	}
	...
	if ( iItemDropRate < iProbabilityA )
	{
		switch ( iChaosCastleIndex )
		{
			...
			case 1: case 2: case 3: case 4: case 5: case 6:
				iItemType = Configs.CCDropID2;
				LogAddTD("[Chaos Castle] (%d) [%s][%s] Winner Item - Gem of Creaion", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
		}
	...
	else
	{
		switch ( iChaosCastleIndex )
		{
			case 0:
				...
				if(Configs.NewServer != FALSE) //season4.5 add-on
				{
					iItemType = Configs.CCDropID1;
