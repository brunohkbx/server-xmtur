...
void CGMMng::Init()
{
	...
	this->cCommand.Add("/templeready", 392, 32); //season 2.5 add-on
	this->cCommand.Add("/templestart", 393, 32); //season 2.5 add-on
	this->cCommand.Add("/templeend", 394, 32); //season 2.5 add-on
	this->cCommand.Add("/templereset", 395, 32); //season 2.5 add-on
	this->cCommand.Add("/masterlevel", 406, 32); //season 3.5 add-on
	this->cCommand.Add("/mlpoint", 407, 32); //season 3.5 add-on
	this->cCommand.Add("/ms_reset", 408, 32); //season 3.5 add-on*/
...
int CGMMng::ManagementProc(LPOBJ lpObj, char* szCmd, int aIndex) //00570A00
{
	...
	case 232://Identical
		...
		case 342:
		{
			...
			MsgOutput(lpObj->m_Index, "[펜릴기본내구력감소] %d -> %d", Configs.FenrirDefaultMaxDurSmall, iTokenNumber);

			Configs.FenrirDefaultMaxDurSmall = iTokenNumber;
		}
		break;
		case 343: //??
		{
			...
			MsgOutput(lpObj->m_Index, "[펜릴요정내구력감소] %d -> %d", Configs.FenrirElfMaxDurSmall, iTokenNumber);

			Configs.FenrirElfMaxDurSmall = iTokenNumber;
		}
		break;
		case 349: //??
		{
...
//
		case 439:
		{
			...
			Configs.AbilityDebug = (iTokenNumber == 1)?1:0; //wtf?

			char szTemp[256];
			wsprintf(szTemp, "Ability Debug: %s", (Configs.AbilityDebug == TRUE) ? "On" : "Off");
		...
		case 202:
		{
			...
			{
				if ( Configs.EnableBattleSoccer != FALSE )
				{
					pId = this->GetTokenString();
					...
					if ( lpObj->VpPlayer[n].type == OBJ_MONSTER )
					{
						if ( lpObj->VpPlayer[n].number >= 0 )
						{
							...
							if ( lpTarget->Class == 275 )
							{
								...
								pNotice.SendToUser(lpObj->m_Index, "쿤둔 HP 초당회복량 = %d 회복량 = %d 회복시간 = %d", Configs.KundunRefillHPSec, Configs.KundunRefillHP, Configs.KundunRefillHPTime);
							}
						}
					}
				}
		...
		case 322:
		{
			...
			Configs.KundunRefillHP = RefillHP;
		...
		case 323:
		{
			...
			Configs.KundunRefillHPSec = RefillHPSec;

			TNotice pNotice(0);
			pNotice.SendToUser(lpObj->m_Index, "쿤둔 HP 초당회복량 = %d 회복량 = %d 회복시간 = %d",
				Configs.KundunRefillHPSec, Configs.KundunRefillHP, Configs.KundunRefillHPTime);
		}
		break;
		case 324:
		{
			...
			Configs.KundunRefillHPTime = RefillHPTime;
