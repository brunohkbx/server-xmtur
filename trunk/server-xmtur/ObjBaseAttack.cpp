...
BOOL CObjBaseAttack::CheckAttackArea(LPOBJ lpObj, LPOBJ lpTargetObj) //Identical gs-cs 56
{
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		if ( iRet != 0 )
		{
			...
			if ( Configs.IsIgnorePacketSpeedHackDetect != FALSE )
			{
				return FALSE;
			}
		}
...
{
	if ( Configs.Language == 0 || Configs.Language == 2 || Configs.Language == 4)
	{
		if ( lpObj->Type == OBJ_USER )
		{
	...
	if ( lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER)
	{
		...
		if ( Configs.Language == 0 || Configs.Language == 2 )
		{
			BOOL bPlayerKiller = FALSE; //Season 2.5 add-on
...
BOOL CObjBaseAttack::MissCheckPvP(LPOBJ lpObj , LPOBJ lpTargetObj, int skill, int skillSuccess, int magicsend, BOOL & bAllMiss)
{
	...
	iAttackSuccessRate = 100.0f * iExpressionA * Configs.SuccessAttackRateOption * iExpressionB;
	...
	if ( dwRate > iAttackSuccessRate )
	{
		...
		if ( Configs.ShieldComboMissOptionOn == TRUE )
		{
