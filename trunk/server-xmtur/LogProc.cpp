...
void LogDataSet()
{
	...
	wsprintf(szTemp,"log\\%02d%02d%02d_%s.log",LogMYear, LogMonth, LogMDay,&Configs.szServerName);
...
void LogAddStrHexFunc(char* str,char* data, int len)
{
	...
	else
	{
		...
		else
		{
			...
			for(n=0; n<len; n++){
				//fprintf(logfp, "0x%02x ", *(BYTE *)(data+n));
				wsprintf(Buff, "%s0x%02X,",Buff,*(BYTE *)(data+n));
			}
...
void LogTextPaintProc(HWND hWnd)
{
	...
	while(total-- != 0)
	{
		...
		case 7:
			...
			break;
		case 8:
			SetTextColor(hdc, RGB(22, 141, 175));
			break;
		default:
