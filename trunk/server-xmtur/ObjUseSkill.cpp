...
BOOL CObjUseSkill::SpeedHackCheck(int aIndex)
{
	...
	if(iTimeCalc < lpObj->m_DetectSpeedHackTime && Configs.RemoveAntiHack == 0)
	{
		lpObj->m_DetectCount++;
		...
		if(lpObj->m_DetectCount > Configs.HackCheckCount)
		{
			...
			if(Configs.IsKickDetecHackCountLimit != 0 && lpObj->m_DetectedHackKickCount > Configs.DetectedHackKickCount)
			{
	...
	if(Configs.IsIgnorePacketSpeedHackDetect != 0 && lpObj->m_SpeedHackPenalty > 0 && Configs.RemoveAntiHack == 0)
	{
...
BOOL CObjUseSkill::EnableSkill(DWORD Skill, int Level) //004E9E10
{
	if(g_MasterSkillSystem.CheckRequireStatus(Skill) != FALSE) //Season3 add-on
	{
		...
		&& Skill != 237
		&& Skill != 238
		&& Skill != AT_SKILL_LARGE_RING_BLOWER //New Rage Fighter Skills
...
void CObjUseSkill::UseSkill(int aIndex, CMagicInf * lpMagic, BYTE x, BYTE y, BYTE dir, BYTE TargetPos, int aTargetIndex) //004BECB0
{
	...
	if(usemana >= 0)
	{
		...
		else if(lpMagic->m_Skill == AT_SKILL_DRAGON_LORE)
		{
			this->SkillDragonLore(aIndex,aTargetIndex,lpMagic);
		}
...
BOOL CObjUseSkill::RunningSkill(int aIndex, int aTargetIndex, CMagicInf * lpMagic, BOOL bCombo) //
{
	...
	case AT_SKILL_CALLMON1:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon1,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON2:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon2,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON3:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon3,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON4:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon4,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON5:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon5,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON6:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon6,lpObj->X-1,lpObj->Y+1);
	case AT_SKILL_CALLMON7:	return this->SkillMonsterCall(lpObj->m_Index,Configs.Summon7,lpObj->X-1,lpObj->Y+1);
	...
	//Rage Fighter
	case AT_SKILL_DEF_SUCC_RATE: this->SkillDefenseSuccessRate(aIndex,lpMagic);break;
	case AT_SKILL_FITNESS: this->SkillFitness(aIndex,lpMagic);break;
	case AT_SKILL_REDARMOR_IGNORE: this->SkillRedArmorIgnore(aIndex,lpMagic);break;
	...
	case AT_SKILL_CHAIN_DRIVE: this->SkillChainDriver(aIndex,aTargetIndex,lpMagic);break;
	case AT_SKILL_DARKSIDE: this->SkillDarkSide(aIndex,aTargetIndex,lpMagic);	break;
...
void CObjUseSkill::KnightSkillAddLife(int aIndex, int skill_level) //
{
	...
	int iaddLifepower = Configs.Inner_Base + (lpObj->Vitality + lpObj->AddVitality) / Configs.Inner_Vitality + (lpObj->Energy + lpObj->AddEnergy)/ Configs.Inner_Energy + viewpercent;
	int iLifeTime = Configs.Inner_Timer + (lpObj->Energy + lpObj->AddEnergy) / Configs.Inner_Timer_Divisor;
	int iaddlife;
...
void CObjUseSkill::WizardMagicDefense(int aIndex, int aTargetIndex, int skill_level) //004C0560
{
	...
	WizardSkillDefense = 10 + (lpObj->Dexterity + lpObj->AddDexterity) / Configs.MS_Agility + (lpObj->Energy + lpObj->AddEnergy) / Configs.MS_Energy;
	WizardSkillDefenseTime = 60 + (lpObj->Energy + lpObj->AddEnergy) / Configs.MS_Divisor;
...
BOOL CObjUseSkill::SkillChangeUse(int aIndex) //
{
	...
	switch(skill_level)
	{
	case 0: change = Configs.Ring1;	break;
	case 1: change = Configs.Ring2; break;
	case 2: change = Configs.Ring3;	break;
	case 3: change = Configs.Ring4;	break;
	case 4:	change = Configs.Ring5;	break;
	case 5:	change = Configs.Ring6;	break;
	...
	case 503:	change = skill_level;	break; //season 4
	case 548:	change = skill_level;	break; //season 4
...
int CObjUseSkill::SkillDamageReflection(int aIndex,int aTargetIndex,CMagicInf * lpMagic) //004C8CF0
{
	...
	{
		int iValue = Configs.ReflectRate + (lpObj->Energy / Configs.ReflectDivisor);
...
int CObjUseSkill::SkillBerserker(int aIndex, CMagicInf* lpMagic) //004F74C0
{
	...
	int iDuration = lpObj->Energy / Configs.Berserker_EnergyDivisor + Configs.Berserker_EnergyAdd;

	int iIncMana = lpObj->Energy / Configs.Berserker_ManaRate;
	int iDecLife = lpObj->Energy / Configs.Berserker_Timer;
...
void CObjUseSkill::SkillDefenseSuccessRate(int aIndex, CMagicInf * lpMagic)
{
	...
	if(partynum == -1)
	{
		...
		gObjApplyBuffEffectDuration(lpObj, AT_DEFENSE_SUCCESS_RATE, 33, iDefense, 0, 0, iDefenseTime);
		GCMagicAttackNumberSend(lpObj,AT_SKILL_DEF_SUCC_RATE,lpObj->m_Index,skillSuccess);
	}
	else
	{
		for(int n = 0; n < MAX_USER_IN_PARTY; n++)
		{
			if(ApplyPartyIndex[n]!= -1)
			{
				...
				gObjApplyBuffEffectDuration(lpPartyObj, AT_DEFENSE_SUCCESS_RATE, 33, iDefense, 0, 0, iDefenseTime);
				GCMagicAttackNumberSend(lpObj,AT_SKILL_DEF_SUCC_RATE,lpPartyObj->m_Index,skillSuccess);
			}
		}
	}
}
...
void CObjUseSkill::SkillFitness(int aIndex, CMagicInf * lpMagic)
{
	...
	int iFitnessTime = 60 + (lpObj->Energy + lpObj->AddEnergy) / 10;
	int iAddLife = 0;
	if(partynum == -1)
	{
		iAddLife = int(lpObj->MaxLife) * iFitnessPower / 100;
		gObjApplyBuffEffectDuration(lpObj, AT_FITNESS, 33, iFitnessPower, 0, 0, iFitnessTime);
	...
	else
	{
		for(int n = 0; n < MAX_USER_IN_PARTY; n++)
		{
			if(ApplyPartyIndex[n]!= -1)
			{
				...
				gObjApplyBuffEffectDuration(lpPartyObj, AT_FITNESS, 33, iFitnessPower, 0, 0, iFitnessTime);
...
void CObjUseSkill::SkillRedArmorIgnore(int aIndex, CMagicInf * lpMagic)
{
	...
	if(partynum == -1)
	{
		gObjApplyBuffEffectDuration(lpObj, AT_IGNORE_DEFENSE, 33, 5, 0, 0, iRedArmorIgnoreTime);
		GCMagicAttackNumberSend(lpObj,AT_SKILL_REDARMOR_IGNORE,lpObj->m_Index,skillSuccess);
	}
	else
	{
		for(int n = 0; n < MAX_USER_IN_PARTY; n++)
		{
			if(ApplyPartyIndex[n]!= -1)
			{
				...
				gObjApplyBuffEffectDuration(lpPartyObj, AT_IGNORE_DEFENSE, 33, 5, 0, 0, iRedArmorIgnoreTime);
				GCMagicAttackNumberSend(lpObj,AT_SKILL_REDARMOR_IGNORE,lpPartyObj->m_Index,skillSuccess);
			}
		}
	}
}
