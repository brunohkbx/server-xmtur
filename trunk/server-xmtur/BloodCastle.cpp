...
{
	Configs.BloodCastle=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "BloodCastleEvent", 0, gDirPath.GetNewPath(COMMONSERVER_FILE));
	Configs.AngelKingsPaperDropRate=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "AngelKingsPaperDropRate", 0, gDirPath.GetNewPath(COMMONSERVER_FILE));
	Configs.BloodBoneDropRate=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "BloodBoneDropRate", 0, gDirPath.GetNewPath(COMMONSERVER_FILE));
	Configs.StoneDropRate=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "StoneDropRate", 0, gDirPath.GetNewPath(COMMONSERVER_FILE));
	Configs.StoneItemDrop=GetPrivateProfileInt(COMMONSERVER_MAINSECTION, "StoneItemDrop", 0, gDirPath.GetNewPath(COMMONSERVER_FILE));
}
...
int CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		if ( gObj[iIndex].Class == CLASS_DARKLORD || gObj[iIndex].Class == CLASS_MAGUMSA )
		{
			gObj[iIndex].LevelUpPoint += Configs.BCDLMGPoints;
		}
		else
		{
			gObj[iIndex].LevelUpPoint += Configs.BCNormalPoints;
		}

		if ( gObj[iIndex].PlusStatQuestClear != false )
		{
			gObj[iIndex].LevelUpPoint += Configs.BCMarlonPoints;
...
void CBloodCastle::DropChaosGem(int iIndex) //00552850
{
	...
	BloodCastleBag.ItemDrop(&gObj[iIndex]);
	return;
...
void CBloodCastle::SendRewardScore(int iIndex, int iSCORE, int iLeftTime, int iAlivePartyCount)
{
	...
	{
		cRankingServer.Close();
		cRankingServer.CreateSocket(ghWnd);
	...
	{
		cRankingServer.DataSend((char *)&pMsg, pMsg.h.size);
	}
}
