...
{
	this->m_hFont = CreateFont(75, 0, 0, 0, FW_THIN, FALSE, FALSE, FALSE, 
		ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY,
		DEFAULT_PITCH|FF_DONTCARE, "Times");

	this->m_hNormalBrush = CreateSolidBrush(RGB(0, 0, 0));
...
{
	if ( cJoinServer.GetSocket() == INVALID_SOCKET )
		this->m_bValidJSConnection = FALSE;
	else
...
void TServerInfoDisplayer::PaintAllInfo(HWND hWnd, int iTopLeftX, int iTopLeftY)
{
	...
	TextOut(hDC, 20, 20, ServerTypeText[0], strlen(ServerTypeText[0]));
