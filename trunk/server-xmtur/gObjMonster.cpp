...
BOOL gObjMonsterRegen(LPOBJ lpObj)
{
 ...
 //HermeX
	...
	else if ( g_CrywolfSync.GetOccupationState() == 0 && Configs.CrywolfApplyMvpBenefit != 0 )
	{
...
int gObjGuardSearchEnemy(LPOBJ lpObj)
{
	...
	for (n=0;n<MAX_VIEWPORT_MONSTER;n++)
	{
		...
		if ( tObjNum >= 0 )
		{
			...
			if( gObj[tObjNum].PartyNumber >= 0 && Configs.FixRemovePKPartySystem == 0) //Season 2.5 add-on
			{
			...
			if (lpObj->Class == 249 || lpObj->Class == 247 )
			{
				...
				if(!(rand()%30)){
					if(lpObj->m_PK_Count == 0){
						ChatSend(lpObj, "Bienvenidos a MU Extreme Season5 EP4!");
						lpObj->m_PK_Count = 0;
					}
				}
...
void gObjRefillMonsterHP(LPOBJ lpMonsterObj, int iRefillHPSec)
{
	...
	if(lpMonsterObj->Class == 275)
	{
		...
		if(lpMonsterObj->iObjectSecTimer > Configs.KundunRefillHPTime)
		{
			iRefillHP = rand() % ((Configs.KundunRefillHP > 0)? Configs.KundunRefillHP : 1) ;

			lpMonsterObj->iObjectSecTimer = 0;

			KUNDUN_EVENT_LOG.Output("[KUNDUN] HP Log -> [%d]회복  [%d]회복시간  [%7.0f]현재HP",	iRefillHP, Configs.KundunRefillHPTime, lpMonsterObj->Life);
			lpMonsterObj->iObjectSecTimer = 0;
		}
		else
		{
		...
		if(lpMonsterObj->iObjectSecTimer % Configs.KundunHPLogSaveTime == 0)
		{
		...
		if(!g_CrywolfSync.GetOccupationState())
		{
			...
			{
				if(Configs.CrywolfApplyMvpBenefit!=0)
				{
					iRefillHP = 0;
					iRefillHPSec = 0;
				}
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if(lpObj->m_Index < OBJ_STARTUSERINDEX && lpObj->Class >= 0){
		ExtremePoints.AddPointsPerMonster(lpTargetObj->m_Index,lpObj->Class);
	}

	if(lpTargetObj->m_Index >= OBJ_STARTUSERINDEX){

		//Season5 Quests Check
		for(int i=0; i < MAX_ACCEPT_QUEST;i++){

			if(lpTargetObj->QuestInfo[i].Type > -1 && lpTargetObj->QuestInfo[i].Index > -1){
				
				int QuestID = QuestManager.GetQuestID(lpTargetObj->QuestInfo[i].Type,lpTargetObj->QuestInfo[i].Index);
				if(QuestID == -1) continue;

				QuestInfo * lpQuest = &QuestManager.QuestList[QuestID];

				for(int u=0; u < 5;u++){
					if(lpQuest->Requirements[u].Type == 1
					&& lpQuest->Requirements[u].ItemType == lpObj->Class
					&& lpQuest->Requirements[u].Count > lpTargetObj->QuestInfo[i].Requirement[u]){
						lpTargetObj->QuestInfo[i].Requirement[u] += 1;
					}
				}
			}
		}
	...
	if(lpObj->Class == 67 && (lpObj->MapNumber == 0 || lpObj->MapNumber == 8 || lpObj->MapNumber == 38)){
		InfernoEvent.DropItem(lpTargetObj->m_Index);
		return;
	}

	//Piece of Speculun Drop Rate
	if((rand()%10000) < Doppelganger.PieceofSpeculumDropRate && lpObj->Level >= 32){
		ItemSerialCreateSend(lpObj->m_Index,lpObj->MapNumber,lpObj->X,lpObj->Y,ITEMGET(14,110),0,1,0,0,0,lpTargetObj->m_Index,0,0);
		return;
	}

	//Piece of Paper Drop Rate
	if((rand()%10000) < ImperialGuardian.PieceOfPaperDropRate && lpObj->Level >= 32){
		ItemSerialCreateSend(lpObj->m_Index,lpObj->MapNumber,lpObj->X,lpObj->Y,ITEMGET(14,101),0,1,0,0,0,lpTargetObj->m_Index,0,0);
		return;
	}

	//Fix Zen for Master's
	if(lpTargetObj->ChangeUP3rd == 1 && lpTargetObj->Level >= 400 && lpTargetObj->PartyNumber == -1){
		lpObj->Money = (lpObj->Money * Configs.ML_AddZen);
	}

	//Panda Ring +50%
	/*if((gObj->pInventory[10].m_Type == ITEMGET(13,76) && gObj->pInventory[10].m_Durability > 0.0f) 
		|| (gObj->pInventory[11].m_Type == ITEMGET(13,76) && gObj->pInventory[11].m_Durability > 0.0f)){
		gTargetObj->Money += ((gTargetObj->Money * 50) / 100);
	}*/
	...
	{
		if ( Configs.CrywolfMonsterDarkElfItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= Configs.CrywolfMonsterDarkElfItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	{
		if ( Configs.CrywolfBossMonsterItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= Configs.CrywolfBossMonsterItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	{
		if ( Configs.KanturuMayaHandItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= Configs.KanturuMayaHandItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	{
		if (Configs.KanturuNightmareItemDrop )
		{
			int ItemDropRate = rand()%10000;

			if ( ItemDropRate <= Configs.KanturuNightmareItemDropRate )
			{
				int iMaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	{
		if (Configs.RaklionSelupanItemDrop != 0)
		{
			int ItemDropRate = rand()%10000;
	...
	if ( lpObj->Class == 275 ) 
	{
		...
		int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
		int iMaxNumOfRewardItem = Configs.KundunItemDrop;
		...
		for ( int i=0;i<iMaxNumOfRewardItem;i++)
		{
			...
			if ( lpObj->Class == 275 ) 
			{
				if ( (rand()%10000) < Configs.KundunAncientDrop )
				{
	...
	if(lpObj->Class == 501)//Season 4.5 REMAKE
	{
		...
		for(int i = 0; i < Configs.GreatGoldenDragonCountDrop;i++)
		{
			if(!gObjGetRandomItemDropLocation(lpObj->MapNumber, cDropX, cDropY, 2, 2, 10)){
				cDropX = lpObj->X;
				cDropY = lpObj->Y;
	...
	ExtDropPer = rand()%Configs.ExcDropRate;	// Excellent Drop Percent #warning
	int ItemDropPer = Configs.ItemDropPer;
	ItemDropPer += Configs.ItemDropPer * lpTargetObj->SetOpImproveItemDropRate / 100;
	...
	if ( item_drop )
	{
		...
		if ( g_CrywolfSync.GetOccupationState() == 1 && Configs.CrywolfApplyMvpPenalty )
		{
			// Jewels
	...
	if ( item_drop)
	{
		...
		if ( ExtDropPer == TRUE )
		{
			option1rand=Configs.ExcLuckRate;
			option2rand=Configs.ExcSkilRate;
			option3rand=rand()%100;
			optionc=rand()%3;
			NOption = NewOptionRand(lpObj->Level);
			level = 0;
		...
		{
			option1rand=Configs.NormalLuckRate;
			option2rand=Configs.NormalSkillRate;
			option3rand=rand()%100;
			optionc=rand()%3;
			NOption = 0;
		}
	...
	if ( Configs.Event1ItemDropTodayPercent > 0 )
	{
		if ( (rand()%Configs.Event1ItemDropTodayPercent) == 0 )
		{
			if ( Configs.Event1ItemDropTodayCount < Configs.Event1ItemDropTodayMax )
			{
				Configs.Event1ItemDropTodayCount++;
...
BOOL gEventMonsterItemDrop(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( Configs.FenrirStuffItemDrop )
	{
		Option1=0;
		Option2=0;
		Option3=0;
		...
		if ( lpObj->Level >= Configs.FenrirStuff_01_DropLv_Min && lpObj->Level <= Configs.FenrirStuff_01_DropLv_Max && lpObj->MapNumber == Configs.FenrirStuff_01_DropMap && bDropStuff == FALSE )
		{
			if ( (rand()%10000) < Configs.FenrirStuff_01_DropRate ) 
			{
				bDropStuff = TRUE;
		...
		if ( lpObj->Level >= Configs.FenrirStuff_02_DropLv_Min && lpObj->Level <= Configs.FenrirStuff_02_DropLv_Max && lpObj->MapNumber == Configs.FenrirStuff_02_DropMap && bDropStuff == FALSE )
		{
			if ( (rand()%10000) < Configs.FenrirStuff_02_DropRate ) 
			{
				bDropStuff = TRUE;
		...
		if ( lpObj->Level >= Configs.FenrirStuff_03_DropLv_Min && lpObj->Level <= Configs.FenrirStuff_03_DropLv_Max && lpObj->MapNumber == Configs.FenrirStuff_03_DropMap && bDropStuff == FALSE )
		{
			if ( (rand()%10000) < Configs.FenrirStuff_03_DropRate ) 
			{
				bDropStuff = TRUE;
	...
	if ( Configs.CompoundPotionDropOn == TRUE )
	{
		BOOL bDropStuff = FALSE;
		int iType = 0;

		if ( lpObj->Level >= Configs.CompoundPotionLv1DropLevel && (rand()%10000) <= Configs.CompoundPotionLv1DropRate && bDropStuff == FALSE )
		{
			bDropStuff = TRUE;
			iType = ItemGetNumberMake(14, 38);
		}

		if ( bDropStuff == FALSE && lpObj->Level >= Configs.CompoundPotionLv2DropLevel && (rand()%10000) <= Configs.CompoundPotionLv2DropRate  )
		{
			bDropStuff = TRUE;
		...
		if ( !bDropStuff )
		{
			if ( lpObj->Level >= Configs.CompoundPotionLv3DropLevel && (rand()%10000) <= Configs.CompoundPotionLv3DropRate )
			{
				bDropStuff = TRUE;
	...
	if ( Configs.IsItemDropRingOfTransform )
	{
		if ( (rand()%10000) < Configs.ItemDropRingOfTransform )
		{
	...
	if ( Configs.FireCrackerEvent ) 
	{
		if ( Configs.Language == 0 )	// If Korea
		{
			if ( lpObj->MapNumber < 7 )
			{
				...
				if ( lpattr )
				{
					...
					{
						if ( (rand()%10000) < Configs.FireCrackerDropRate )
						{
							dur = 255.0f;
							x = lpObj->X;
							y = lpObj->Y;
	...
	if ( Configs.HeartOfLoveEvent )
	{
		if ( Configs.Language == 2 )
		{
			LPMONSTER_ATTRIBUTE lpattr = gMAttr.GetAttr(lpObj->Class);
			...
			if ( lpattr )
			{
				...
				{
					if ( (rand()%10000) < Configs.HeartOfLoveDropRate )
					{
						dur = 255.0f;
						x = lpObj->X;
						y = lpObj->Y;
	...
	if ( Configs.MedalEvent )
	{
		if ( lpObj->MapNumber == 1 || lpObj->MapNumber == 2 )
		{
			if ( (rand()%10000) < Configs.SilverMedalDropRate )
			{
				dur = 255.0f;
				x = lpObj->X;
				y = lpObj->Y;
		...
		{
			if ( (rand()%10000) < Configs.GoldMedalDropRate )
			{
				dur = 255.0f;
				x = lpObj->X;
				y = lpObj->Y;
	...
	if ( Configs.EventChipEvent )
	{
		if ( (rand()%10000) < Configs.BoxOfGoldDropRate )
		{
			dur = 255.0f;
			x = lpObj->X;
			y = lpObj->Y;
	...
	if ( (rand()%10000) < Configs.KundunMarkDropRate )
	{
		Option1=0;
		Option2=0;
		Option3=0;
	...
	if ( (rand()%10000) < Configs.MarkOfTheLord )
	{
		Option1=0;
		Option2=0;
		Option3=0;
	...
	{
		if ( lpObj->Level >= Configs.SleeveOfLordDropLevel )
		{
			if ( (rand()%10000) < Configs.SleeveOfLordDropRate)
			{
		...
		if ( lpObj->Level >= Configs.SoulOfDarkHorseropLevel )
		{
			if ( (rand()%10000) < Configs.SoulOfDarkHorseDropRate)
			{
		...
		if ( lpObj->Level >= Configs.SoulOfDarkSpiritDropLevel )
		{
			if ( (rand()%10000) < Configs.SoulOfDarkSpiritDropRate )
			{
				type = ItemGetNumberMake(13,31);
				level = 1;
				x = lpObj->X;
	...
	if ( Configs.IsDropGemOfDefend )
	{
		...
		{
			if ( lpObj->Level >= Configs.GemOfDefendDropLevel )
			{
				if ( (rand()%10000)< Configs.GemOfDefendDropRate )
				{
	...
	{
		if ( (rand()%10000)< Configs.CondorFlameDropRate )
		{
	...
	if (Configs.IsDropSetItemInCastleHuntZone !=0)	//Land of Trials
	{
		...
		{
			if ( lpObj->Level >= Configs.SetItemInCastleHuntZoneDropLevel ) //Good
			{
				if ( (rand()%10000) < Configs.SetItemInCastleHuntZoneDropRate )
				{
					int MaxHitUser = gObjMonsterTopHitDamageUser(lpObj);
	...
	if ( Configs.KanturuSpecialItemDropOn )
	{
		if ( lpObj->MapNumber == MAP_INDEX_KANTURU2 )
		{
			if ( (rand()%10000) < Configs.KanturuMoonStoneDropRate )
			{
		...
		{
			if ( (rand()%10000) < Configs.KanturuJewelOfHarmonyDropRate )
			{
	...
	if ( lpObj->MapNumber == MAP_INDEX_AIDA || lpObj->MapNumber == MAP_INDEX_CRYWOLF_FIRSTZONE)
	{
		...
		switch ( lpObj->Class )
		{
			...
			case 307:	case 308:	case 309:
				if ( (rand()%10000) < Configs.MysteriousBeadDropRate1 )
					bDropMysteriousBead = true;
				break;

			case 310:	case 311:	case 312:	case 313:
				if ( (rand()%10000) < Configs.MysteriousBeadDropRate2 )
					bDropMysteriousBead = true;
				break;
		}
	...
	if(Configs.DevilSquareEvent ) //season3 add-on (Master Level Devil Square)
	{
		if ( !DS_MAP_RANGE(lpObj->MapNumber ) )
		{
			if ( (rand()%10000) < Configs.ML_EyesOfDevilSquareDropRate )
			{
				if ( lpObj->Level >= 90 )
				{
					Option1=0;
			...
			if ( (rand()%10000) < Configs.ML_KeysOfDevilSquareDropRate )
			{
				if ( lpObj->Level >= 90 )
				{
					Option1=0;
	...
	if ( g_iIllusionTempleEvent ) //Season 3 add-on (Master Level Illusion)
	{
		if ( !IT_MAP_RANGE(lpObj->MapNumber ) )
		{
			if ( (rand()%10000) < Configs.ML_OldScrollDropRate )
			{
				if ( lpObj->Level >= 96)
				{
					Option1=0;
			...
			if ( (rand()%10000) < Configs.ML_CovenantOfIllusionDropRate )
			{
				if ( lpObj->Level >= 96)
				{
					Option1=0;
	...
	if ( Configs.BloodCastle ) //Season 3 add-on (Master Level Blood Castle)
	{
		if ( !BC_MAP_RANGE(lpObj->MapNumber ) )
		{
			if ( (rand()%10000) < Configs.ML_AngelKingsPaperDropRate )
			{
				if ( lpObj->Level >= 90)
				{
					Option1=0;
			...
			if ( (rand()%10000) < Configs.ML_BloodBoneDropRate )
			{
				if ( lpObj->Level >= 90)
				{
					Option1=0;
	...
	if ( Configs.BloodCastle )
	{
		if ( !BC_MAP_RANGE(lpObj->MapNumber ) )
		{
			...
			if ( (rand()%10000) < Configs.AngelKingsPaperDropRate )
			{
				dur = 128.0f;
				x = lpObj->X;
				y = lpObj->Y;
			...
			if ( (rand()%10000) < Configs.BloodBoneDropRate )
			{
				dur = 128.0f;
				x = lpObj->X;
				y = lpObj->Y;
	...
	if ( (rand()%10000) < Configs.DarkLordHeartDropRate )
	{
		int iDropRate = 0;
	...
	if (Configs.RibbonBoxEvent )
	{
		if ( lpObj->Level >= Configs.RedRibbonBoxDropLevelMin && lpObj->Level <= Configs.RedRibbonBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.RedRibbonBoxDropRate )
			{
		...
		else if ( lpObj->Level >= Configs.GreenRibbonBoxDropLevelMin && lpObj->Level <= Configs.GreenRibbonBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.GreenRibbonBoxDropRate )
			{
		...
		else if ( lpObj->Level >= Configs.BlueRibbonBoxDropLevelMin && lpObj->Level <= Configs.BlueRibbonBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.BlueRibbonBoxDropRate )
			{
	...
	if ( Configs.ChocolateBoxEvent )
	{
		if ( lpObj->Level >= Configs.PinkChocolateBoxDropLevelMin && lpObj->Level <= Configs.PinkChocolateBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.PinkChocolateBoxDropRate )
			{
		...
		else if ( lpObj->Level >= Configs.RedChocolateBoxDropLevelMin && lpObj->Level <= Configs.RedChocolateBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.RedChocolateBoxDropRate )
			{
		...
		else if ( lpObj->Level >= Configs.BlueChocolateBoxDropLevelMin && lpObj->Level <= Configs.BlueChocolateBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.BlueChocolateBoxDropRate )
			{
	...
	if ( Configs.CandyBoxEvent )
	{
		if ( lpObj->Level >= Configs.LightPurpleCandyBoxDropLevelMin && lpObj->Level <= Configs.LightPurpleCandyBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.LightPurpleCandyBoxDropRate )
			{
				type = ItemGetNumberMake(14,32);
				level = 1;
				x = lpObj->X;
		...
		else if ( lpObj->Level >= Configs.VermilionCandyBoxDropLevelMin && lpObj->Level <= Configs.VermilionCandyBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.VermilionCandyBoxDropRate )
			{
				type = ItemGetNumberMake(14,33);
				level = 1;
				x = lpObj->X;
		...
		else if ( lpObj->Level >= Configs.DeepBlueCandyBoxDropLevelMin && lpObj->Level <= Configs.DeepBlueCandyBoxDropLevelMax )
		{
			if ( (rand()%10000) < Configs.DeepBlueCandyBoxDropRate )
			{
				type = ItemGetNumberMake(14,34);
				level = 1;
				x = lpObj->X;
	...
	if ( Configs.HallowinDayEventOn )
	{
		...
		if ( !bIsBossMonster )
		{
			if ( (rand()%10000) < Configs.HallowinDayEventItemDropRate  )
			{
	...
	if( Configs.SantaPolymorphRingDropOn ) //season 2.5 add-on
	{
		bool bIsBossMonster = false;
		...
		if ( !bIsBossMonster )
		{
			if ( (rand()%10000) < Configs.SantaPolymorphRingDropRate  )
			{
	...
	if( Configs.CherryBlossomEventOn ) //season 3 add-on
	{
		bool bIsBossMonster = false;
		...
		if ( !bIsBossMonster )
		{
			if ( (rand()%10000) < Configs.CherryBlossomEventItemDropRate  )
			{
	...
	if( Configs.LuckyCoinEventOn != 0)// Season 4.5 Addon
    {
        if ( (rand()%10000) < Configs.LuckyCoinDropRate  )
        {
            Option1=0;
            Option2=0;
            Option3=0;
...
void CQeustNpcTeleport::Run(int aIndex)
{
	...
	if ( this->TimeCount > Configs.QuestNPCTeleportTime )
	{
		...
		LogAdd("NPC %d Teleport", tableindex);
	}
}