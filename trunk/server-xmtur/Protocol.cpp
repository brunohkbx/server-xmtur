...
void ProtocolCore(BYTE protoNum, BYTE * aRecv, int aLen, int aIndex, BOOL Encrypt, int serial)
{
	...
	if (serial >= 0 && Configs.RemoveAntiHack == 0)
	{
	...
	else
	{
		...
		switch ( protoNum )
		{
			...
			case 0x87:
				...
				break;
			case 0x8E: 
				WarpSystem.MovePlayer(aIndex,aRecv[8]);		   
				break;
			case 0x90:
					...
					switch ( lpDef->subcode )
					{
						...
						case 0x17:
							if(lpObj->MapNumber != 79){ 
								gObjMoveGate(aIndex,333);  //Move to Any Place->Loren Market
							} else { 
								gObjMoveGate(aIndex,17); //Move to Loren Market->Lorencia
							} 
						break;
					}

					//Doppelganger Event Enter
					if(aRecv[1] == 0x05 && aRecv[3] == 0x0E){
						Doppelganger.EnterDoppelganger(aIndex,(int)aRecv[4]+12);
						return;
					}
				}
				break;

			/*Season6 Attack Protocol
			case 0x4A:
				if(DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE){
					CGNewMagicAttack((PMSG_NEW_MAGICATTACK *)aRecv, aIndex);
				}
			break;

			case 0x4B:
				GCMonkDarkSideTargetSelect((PMSG_NEWATTACK_S6*)aRecv,aIndex);
			break;*/


			case 0xF6: //Season5 Quest System
				QuestManager.QuestProtocolCore(aIndex,aRecv);
			break;

			case 0xF7: //Imperial Guardian Event
				ImperialGuardian.EnterEvent(aIndex); 
			break;

			case 0xF8: //Season5 Gens System
				GensSystem.GensProtocolCore(aIndex,aRecv);
				return;
			break;
		
			default:
				LogAddC(2, "[ProtocolError][%s][%s] Head: %02X (%s,%d) State:%d", gObj[aIndex].AccountID, gObj[aIndex].Name, protoNum, __FILE__, __LINE__, gObj[aIndex].Connected);
				CloseClient(aIndex);
			break;
		}	
	}
	...
	if(Configs.RemoveAntiHack == 0){
		...
		if ( gObj[aIndex].Connected == PLAYER_PLAYING )
		{
			...
			if ( gObj[aIndex].m_AttackSpeed < lpClientTime->AttackSpeed || gObj[aIndex].m_MagicSpeed < lpClientTime->MagicSpeed )
			{
				...
				LogAddC(2, "[%s][%s][%s] ClientHack Detected CheckSpeedHack : [%s]", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Ip_addr,(Configs.CheckSpeedHack != FALSE) ? "TRUE" : "FALSE");
	
				if(Configs.CheckSpeedHack == FALSE){
					return;
				}
...
void CGCheckMainRecv(PMSG_CHECK_MAINEXE_RESULT * lpMsg, int aIndex) 
{
	...
	if(Configs.dwgCheckSum[gObj[aIndex].CheckSumTableNum] != lpMsg->m_dwKey)
	{
		LogAddTD("error-L1 : CheckSum-Exe error %d %d %d [%s]",Configs.dwgCheckSum[gObj[aIndex].CheckSumTableNum],
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex) 
{
	...
	if (Configs.WriteChatLog )
	{
		...
		pChatMsg.wServer = Configs.GameServerCode;
	...
	else if ( lpChat->chatmsg[0] == '@' )
	{
		if ( lpObj->GuildNumber > 0 )
		{
			...
			else if ( lpChat->chatmsg[1] == '@' && lpObj->lpGuild->iGuildUnion )
			{
				...
				if ( Configs.ServerGroupUnionChatting == TRUE )
			...
			else
			{
				...
				if ( Configs.ServerGroupGuildChatting == TRUE )
				{
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex) 
{
	...
	if ( Configs.CanConnectMember == TRUE )
	{
	...
	cJoinServer.DataSend((char*)&spMsg, spMsg.h.size);
...
void GCJoinBillCheckSend(LPSTR AccountId, int aIndex) 
{
	...
	cJoinServer.DataSend((char*)&pMsg, pMsg.h.size);
}
...
void GCJoinSocketResult(BYTE result, SOCKET Socket) 
{
	...
	cGameServer.DataSocketSend(Socket, (char*)&pResult, pResult.h.size);
}
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type) //004427B0
{
	...
	if ( lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(12,36) && //Season 2.5 add-on
		 ...
		 Configs.PkLimitFree == 0) //season 4.5 add-on
	{
		pResult.Result = false;
	}

	
	if ( lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(12,180) && //Season 2.5 add-on
		 lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(12,183) &&
		 Configs.PkLimitFree == 0) //season 4.5 add-on
	{
		pResult.Result = false;
	}
			...
			else if ( type == ITEMGET(12,26) )
			{
				switch ( level )
				{
					...
					case 5:
						...
						if ( (rand()%10000) < Configs.HiddenTreasureBoxOfflineRate )
						{
...
void CGTalkRequestRecv(PMSG_TALKREQUEST * lpMsg, int aIndex) 
{
	...
	if ( Configs.MerryXMasNpcEvent == TRUE )
	{
		if ( (rand()%6) == 0 )
		{
	...
	if ( Configs.HappyNewYearNpcEvent == TRUE )
	{
		if ( (rand()%6) == 0 )
		{
	...
	else if ( ShopNum == 101 )
	{
		if (Configs.CanChaosBox == TRUE )
		{
			...
			pResult.level1 = Configs.DQChaosSuccessRateLevel1;
			pResult.level2 = Configs.DQChaosSuccessRateLevel2;
			pResult.level3 = Configs.DQChaosSuccessRateLevel3;
			pResult.level4 = Configs.DQChaosSuccessRateLevel4;
			pResult.level5 = Configs.DQChaosSuccessRateLevel5;
			pResult.level6 = Configs.DQChaosSuccessRateLevel6;
			pResult.level7 = Configs.DQChaosSuccessRateLevel7; //season 3.0 add-on
				...
				if( bNoItem )
				{
					...
					while(iCount <= ShopC[tShop].ItemCount){
						if(lpMsg->Pos == ShopC[tShop].SendItemData[iCount+(12*iCount)]){
							pResult.Result = GamblingSystemItemBagOpen(lpObj,235,lpObj->X,lpObj->Y,iCount);
							break;
						}
						iCount++;
					}
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos) 
{
	...
	if ( itemtype >= ITEMGET(14,0) || ( itemtype >= ITEMGET(13,0) && itemtype  < ITEMGET(13,4) ) || 
		...
		(itemtype < ITEMGET(12,180) && itemtype > ITEMGET(12,183)) ||
...
void CGPartyRequestRecv(PMSG_PARTYREQUEST * lpMsg, int aIndex) 
{
	...
	if(PartySystem.CheckLevelResetsDif(aIndex,number) == TRUE){
		return;
	}
	...
	if ( !strcmp(gObj[aIndex].lpGuild->Names[0], gObj[aIndex].Name ))
	{
		...
		if ( gObjJoominCheck(aIndex, joomin) == TRUE )
		{
			if ( !strcmp(memberid, gObj[aIndex].Name ))
			{
				...
				if( Configs.CastleGuildDestoyLimit != 0 )
				{
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type) 
{
	...
	if ( Configs.PkLimitFree == FALSE && gObj[aIndex].m_PK_Level >= 6)
	{
		pMsg.Result = 4;
			...
			if ( gObj[n].Type == OBJ_USER )
			{
				if ( gObj[n].Connected > PLAYER_LOGGED )
				{
					if ( gObj[n].Name[0] == lpNode->Names[0][0] )
					{
						...
						{
							if ( Configs.PkLimitFree == FALSE && gObj[n].m_PK_Level >= 6)
							{
								pMsg.Result = 4;
							...
							if ( lpMyNode->Index[n] >= 0 )
							{
								...
								if ( g_call )
								{
									...
									if ( lpMyNode->WarType == 1 )
									{
										...
										if ( Configs.PkLimitFree || gObj[lpMyNode->Index[n]].m_PK_Level < 6 )
										{
							...
							if ( lpMyNode->lpTargetGuildNode->Index[n] >= 0 )
							{
								...
								if ( g_call )
								{
									...
									if ( lpMyNode->lpTargetGuildNode->WarType == 1 )
									{
										...
										if ( Configs.PkLimitFree != 0 || gObj[lpMyNode->lpTargetGuildNode->Index[n]].m_PK_Level < 6 )
										{
...
{
	if ( Configs.CanWarehouseLock == FALSE )
	{
		return;
	}
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex) 
{
	...
	if(aRecv->Type == 3 || aRecv->Type == 4 || aRecv->Type == 22 || aRecv->Type == 23 || aRecv->Type == 49 || aRecv->Type == 50)
	{
		int pos = 0;
	...
	switch ( iMixType )
	{
		...
		case CHAOS_TYPE_IMPERIAL_BOOK:
			g_MixSystem.CreateSemicronBook(lpObj);
		break;
		...
		case CHAOS_TYPE_SEALED_BOX:
			g_MixSystem.SealedBoxMix(lpObj);
		break;

		case CHAOS_TYPE_EXTREMEWINGS: //Season5 CUSTOM
			g_MixSystem.Extreme_CustomWingMixes(lpObj);
		break;
...
void GCMonkDarkSideTargetSelect(PMSG_NEWATTACK_S6 *lpMsg,int aIndex)
{
	LPOBJ lpObj;

	if(gObj[aIndex].CloseCount >= 0){
		LogAdd("[GCMonkDarkSideTargetSelect]: Clouse count > -1");
		return;
	}
	...
	if(!lpMagic){
		LogAdd("[GCMonkDarkSideTargetSelect]: User Magic Error");
		return;
	}

	PMSG_DURATION_MAGIC_SEND pMsg;

	PHeadSetBE((PBYTE)&pMsg,0x4B,sizeof(pMsg));
...
void CGAttack(PMSG_ATTACK* lpMsg, int aIndex) 
{
	...
	if(Configs.RemoveAntiHack == 0){
		...
		if ( iTimeCalc < lpObj->m_DetectSpeedHackTime) //season4 changed (fix)
		{
			...
			if ( lpObj->m_DetectCount > Configs.HackCheckCount )
			{
				lpObj->m_DetectedHackKickCount++;
				lpObj->m_SpeedHackPenalty = Configs.SpeedHackPenalty;
			
				if ( Configs.IsKickDetecHackCountLimit )
				{
					if ( lpObj->m_DetectedHackKickCount > Configs.DetectedHackKickCount )
					{
	...
	if(Configs.IsIgnorePacketSpeedHackDetect && Configs.RemoveAntiHack == 0)
	{
...
void CGNewMagicAttack(PMSG_NEW_MAGICATTACK* lpMsg, int aIndex)
{
	LPOBJ lpObj;
	LPOBJ lpTargetObj;
	int usernumber = MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL);
	...
	if ( usernumber < 0 || usernumber > OBJMAX-1 )
	{
		LogAdd("[CGMagicAttack]: Index Error -> File: %s, Line: %d, Index: %d", __FILE__, __LINE__, usernumber);
		return;
	}
	...
	{	
		...
		if ( attackret )
		{
			LogAdd("[CGMagicAttack]: [%s][%s] Try Attack In Not Attack Area [Protocol] (%s:%d,%d) errortype = %d",lpObj->AccountID, lpObj->Name, lMsg.Get(MSGGET(7, 208)+lpObj->MapNumber), lpObj->X, lpObj->Y,attackret);
	...
	if(lpObj->Type == OBJ_USER ){
		...
		int Energy = lpObj->Energy + lpObj->AddEnergy; //season 3.5 add-on
		int Strength = lpObj->Strength + lpObj->AddStrength; //season 3.5 add-on
		int Dexterity = lpObj->Dexterity + lpObj->AddDexterity; //season 3.5 add-on 
	...
	if ( !gCheckSkillDistance(aIndex, usernumber, lpMagic->m_Skill))
	{
		...
		if ( dwTick > (Configs.SkillDiatanceKickCheckTime*1000) )
		{
		...
		if ( lpObj->m_iSkillDistanceErrorCount > Configs.SkillDistanceKickCount )
		{
			if ( Configs.SkillDistanceKick )
			{
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	if ( attackret )
	{
		...
		if(Configs.IsIgnorePacketSpeedHackDetect){
			return;
		}
	}
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex) 
{
	...
	if(IMPERIAL_MAP_RANGE(gObj[aIndex].MapNumber)){
		ImperialGuardian.CheckPassGates(aIndex,lpMsg->MoveNumber);
	}
...
void CGBeattackRecv(BYTE* lpRecv, int aIndex, int magic_send) 
{
	...
	if ( lpMagic == NULL )
	{
		LogAdd("[lpMagic == NULL] error-L3 %s %d", 
			__FILE__, 
			__LINE__);
		return;
	}
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		if ( Configs.EnableCheckPenetrationSkill )
		{
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex) //0045C690
{
	...
	if ( citem->IsItem() )
	{
			...
			citem->m_Type == ITEMGET(12,48) ) //season4
		{
			...
			{
				int addskill = gObjMagicAdd(&gObj[aIndex], citem->m_Type>>(9), citem->m_Type %  MAX_SUBTYPE_ITEMS, citem->m_Level, (WORD)skillnumber);
...
void GCReqmoveDevilSquare(PMSG_REQ_MOVEDEVILSQUARE* lpMsg, int aIndex) 
{
	...
	if ( bPlayerKiller == TRUE && Configs.EnablePKPlayersInEvents == 0)
	{
		pResult.Result = 6;
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		return;
	...
	if ( ItemLevel == 10 && Configs.FixMasterLevelEvents == 0)
	{
...
void GCReqDevilSquareRemainTime(PMSG_REQ_DEVILSQUARE_REMAINTIME* lpMsg, int aIndex) 
{
	...
	{
		case 1:
			if ( gObj[aIndex].m_PK_Level >= 4 && Configs.EnablePKPlayersInEvents == 0)
			{
...
void CGRequestEnterBloodCastle(PMSG_REQ_MOVEBLOODCASTLE* lpMsg, int iIndex) 
{
	...
	if(gObj[iIndex].PartyNumber >= 0 && Configs.FixRemovePKPartySystem == 0) //Season 2.5 add-on
	{
...
{
	if ( !Configs.EnableServerDivision)
	{
		return;
	}
...
void CGDuelStartRequestRecv(PMSG_REQ_START_DUEL * lpMsg, int aIndex) //maybe identical to gs 56 common "new party shit for pk player"
{
	...
	if ( !Configs.PkLimitFree )
	{
		BOOL bPlayerKiller = FALSE; //Season 2.5 add-on
	
		if(gObj[aIndex].PartyNumber >= 0 && Configs.FixRemovePKPartySystem == 0) //Season 2.5 add-on
		{
...
void CGReqPlusChaosRate(PMSG_REQ_CRYWOLF_BENEFIT_PLUS_CHAOSRATE* lpMsg, int iIndex) 
{
	...
	if(g_CrywolfSync.GetOccupationState() == 0 && Configs.CrywolfApplyMvpBenefit != 0) //season 3.0 add-on
	{
...
void CGCheckNameRequest(PMSG_REQ_CHARACTER_CHECKNAME* lpMsg, int iIndex) //identical
{
	...
	if(Configs.CanChangeCharacterName != FALSE) //serverinfo.dat
	{
