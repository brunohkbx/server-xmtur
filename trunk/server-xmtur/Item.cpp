...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3,  BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx, BYTE SocketOption[5], BYTE SocketIndex, BYTE DbVersion)
{
	...
	if ( gSetItemOption.IsSetItem(_type) != 0 && SetOption != 255 && SOption != 0)
	{
		...
		{
			if(Configs.EnableExcellentWithAncient == 0){
				this->m_NewOption = 0;
			}
			Attribute2 = 0;
	...
	if(g_SocketOption.IsActive(this) == 1)
	{
		...
		{	//Zero SET
			...
			if(Configs.EnableExcellentWithSocket == 0){
				this->m_ItemOptionEx = 0;
				this->m_SetOption = 0;
				Attribute2 = 0;
				this->m_NewOption = 0;
	...
	if ( p->RequireLevel != 0 )
	{
		...
		else if ( _type >= ITEMGET(12,180) && _type <= ITEMGET(12,183) )
		{
			this->m_RequireLevel = p->RequireLevel;
		}
	...
	if ( Option2 != 0 )
	{
		...
		if ( _type >= ITEMGET(12,180) && _type <= ITEMGET(12,183) )	//Season 2.5 Third Wings
		{
			this->m_Special[this->m_SpecialNum] = 84;
			this->m_Option2 = 1;
		}
	...
	if ( Option3 != 0 )
	{
		...
		if ( _type == ITEMGET(12,36) || (_type >= ITEMGET(12,180) && _type <= ITEMGET(12,183)) ) // Wing of Storm
		{
			this->m_Option3 = Option3;
	...
	if ( _type >= ITEMGET(12,36) && _type <= ITEMGET(12,40) || _type == ITEMGET(12,43) || _type >= ITEMGET(12,180) && _type <= ITEMGET(12,183))
	{
		if ( (this->m_NewOption &1) )
		{
...
void CItem::Value()
{
	...
	if(this->m_Type >= ITEMGET(13,71) && this->m_Type <= ITEMGET(13,74)){
		this->m_SellMoney = 2000000;
	}
	...
	{
		Gold = Configs.FruitPrice;
	}
	else if ( this->m_Type == ITEMGET(13,14) ) //Loch Feather
	{
		if ( this->m_Level == 1 )
		{
			Gold = Configs.MonarchPrice;	// Crest ofMonarch
		}
		else
		{
			Gold = Configs.FeatherPrice;	// Blue Feather
		}
	}
	...
	else if ( (Type == 12 && this->m_Type > ITEMGET(12,6) && (this->m_Type < ITEMGET(12,36) && this->m_Type > ITEMGET(12,43))
	&& (this->m_Type < ITEMGET(12,180) && this->m_Type > ITEMGET(12,183))) 
	|| Type == 13 || Type == 15 ) //Third Wings Addition
	{
	...
	else
	{
		...
		else if( Type == 12 && (this->m_Type >= ITEMGET(12,180) && this->m_Type <= ITEMGET(12,183)) ) //Third Wings
		{
			Gold = (Level2+40)*Level2*Level2*11+40000000;
		}
		...
		else if( Type == 12 && (this->m_Type >= ITEMGET(12,180) && this->m_Type <= ITEMGET(12,183)) ) //Summoner Wings
		{
			Gold = (Level2+40)*Level2*Level2*11+40000000;
		}
		else
...
int CItem::IsThirdWingOpGetOnePercentDamage()
{
	...
	|| this->m_Type >= ITEMGET(12,180) && this->m_Type <= ITEMGET(12,183) || this->m_Type == ITEMGET(12,43))
	{
		if ( this->m_Durability < 1.0f )
		{
			return 0;
...
void BufferItemtoConvert3(LPBYTE buf, int& type, BYTE& level, BYTE& op1, BYTE& op2, BYTE& op3, BYTE& dur)
{
	...
	else
	{
		if ( (buf[3] & 0x40) == 0x40 )
		{
			op3 = Configs.JewelOfLifeMaxOption;
		}
	}*/
}
